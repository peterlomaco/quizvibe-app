...
## Aktiv roadmap (uppdaterad 2026-06-02)
...
**Region-tag** (2026-05-26, utökat 2026-06-02): `RegionSchema` har nu TWO värden: `'sweden'` och `'global'`. Alla catalog-filer har `region: ["sweden"]` på fil-headern (V1-default). **`region: ["global"]`** (item-level override) = internationellt erkänd men ej specifikt Sverige-fokuserad — visas EJ för V1-spelaren (filter: item.region ∩ player.region = "sweden" ∩ "global" = ∅). 34 items taggade `global` (2026-06-02): skådespelare som Amy Adams, Anne Hathaway, Chris Evans, idrottare som Iniesta, Jean Tigana, artister som Carole King, Travis Scott m.fl. som ej prioriteras i V1 svensk pool men behåller sin katalog-post för framtida global expansion. **Curator-regel**: alla nya items DEFAULT till fil-headerns `region: ["sweden"]`. Lägg `region: ["global"]` item-level om personen inte har stark svensk igenkänning men är globalt känd. Filter-semantik: visa item om `item.region` intersects `player.region`. Se `memory/project_v1_v2_region_strategy.md` för full spec.
