...
⚠ Det här är BAS-helpers. Den LIVE spelar-progressionsmotorn (streak-skalad + tier-dämpad delta, decay, display) bor i [hcpEngine.ts](src/utils/hcpEngine.ts) + [hcpProgress.ts](src/utils/hcpProgress.ts) — se "Dynamic HCP System" nedan.
...
**Peters designbeslut (2026-08-28, delta-modell omarbetad 2026-09-12):** alla nya spelare startar på **HCP 99** (assistance/ålder-baserat start avfärdat); **fullt 20-svars-fönster** innan första steget, sedan kontinuerligt glidande. Deltan är INTE längre ±1 — den är **streak-skalad och tier-dämpad**: (1) råa STEG ur fönstersumman = ett steg per svar förbi tröskeln (okapat), (2) skalas med en tier-faktor ur HCP:t FÖRE spelet, (3) den UPPÅTGÅENDE deltan cappas per nivå × tier. Se nedan.

**Ren motor** — [src/utils/hcpEngine.ts](src/utils/hcpEngine.ts) (INGA React/AsyncStorage-beroenden; Date injiceras → deterministisk; enhetstestad i [backend/content/test/hcpEngine.test.ts](backend/content/test/hcpEngine.test.ts), 53 tester):
- `HcpProgress = { categories: Record<MainCategory, CategoryProgress> }`; `CategoryProgress = { hcp, windows: Record<AssistanceLevel, boolean[]>, lastPlayedISO }`. `hcp` lagras som FLYTTAL (tier-steg ger 0,25/0,5-delar, decay 0,25); DISPLAY avrundas ALLTID UPPÅT (§1.2.3) via `displayHcp` (ceil + clamp [1,99]).
- `HCP_START = 99`, `HCP_WINDOW_SIZE = 20`. Trösklar (fönstersumma S): Full S≥18 → nedåt / ≤12 → uppåt; Standard ≥16 / ≤10; Minimal ≥14 / ≤8.
- **`evaluateWindow(win, level)` → RÅTT signerat stegantal** (okapat): `−(S − upper + 1)` när S≥upper, `+(lower − S + 1)` när S≤lower, annars 0. Ex FULL: 18/20→−1, 20/20→−3, 12/20→+1, 0/20→+13. Kräver FULLT fönster (20) → ny spelares HCP stabilt ~5 spel.
- **`hcpTier(hcp)` / `hcpTierFactor(hcp)`** — steg-faktor ur HCP:t FÖRE spelet: **>60 → ×0.75, 30–60 → ×0.5, 1–29 → ×0.25** (60.0/30.0 → mid). Gäller BÅDA riktningar.
- **`WORST_DELTA_CAP[level][tier]`** — tak på den UPPÅTGÅENDE (sämre) deltan; nedåt cappas ALDRIG. FULL: 5/3.5/1.75 · Standard: 4/2.5/1.25 · Minimal: 3/2.0/1 (high/mid/low). Ett 0/20-spel ger alltså exakt cappen; en delta under cappen är orörd.
- `applyGameResult` = lägg svar i nivåns fönster → `råa steg × tier-faktor` → cappa om positiv → klampa [1,99] → stämpla lastPlayedISO. `applyInactivityDecay` (§2.4: +0.25 per hel 7-dagarsperiod; flyttar lastPlayed fram med HELA perioder, ej till now, så vecko-resten bevaras + ingen dubbelräkning).
