...
**Urvalsregeln bytte källa i samma pass.** `export-image-questions.ts` valde items på "har en webp i assets/quiz-images/". Nu väljer den via `meetsHintsThreshold` (`hintsText.ts`) — se "Spelbarhets-gate" nedan (aktuellt: **≥8 råa ledtrådar OCH en riktig landsflagga**, Peter 2026-09-03). Webp-beroendet är borta. **Lägg inte tillbaka ett filsystem-beroende i den exporten.**
...
**TV-serier: `mediaType: series` (Peter 2026-09-23).** Serier ligger i samma Film-kategori (`contentSubject: movie`) men är **Name-only** — en schema-refine avvisar `mediaType: series` utan `actor-select` eller med `timeline`. Exporten byter bara ordet: "Select one of the main actors in this **series**?" / "What is the name of the main character in this **series**?" (`isAnimated: true`). Default `mediaType: film` → befintliga filmer oförändrade. I Film.xls: kolumn Type = Film | Series. Engelska: "series" även i singular (inte "serie").
...
5. **Image-items**: INGEN bildfil behövs — bild-assetsen är raderade (se "⚠ Bild-assets är RADERADE"). Ett image-item blir spelbart när det klarar `meetsHintsThreshold` (**≥8 råa hints i `HINTS_LIBRARY` OCH en mappad nationalitet/flagga** — se "Spelbarhets-gate"); kör `npm run fetch-hints-data` och därefter `npm run export-image-questions`.
...
5. **Player history** — [PlayerHistorySection.tsx](src/components/PlayerHistorySection.tsx) manages its own collapse state. **Per-spel-kort (2026-09-11, ERSÄTTER månads-grupperade statiska rader):** varje spel är ett eget TAPPBART kort som öppnar spelets **final leaderboard** (alla spelares rader via `buildAggregateStandings({games:[{roomCode, players}]}) → finalizeRows(…, false) → LeaderboardTable`, EXAKT samma väg som Marathon-listans detalj-modal — skivad till EN spel-serie). Grupperingen delas med Marathon-listan via [historyGrouping.ts](src/utils/historyGrouping.ts): **Date**-sort = månad → dag → spelform (tre-nivå, `groupHistoryByMonthDateForm`), **Host Name**-sort = host → spelform (`groupHistory`), plus samma QuizVibe-friend-filter (utbruten till delad [PlayerFilterPicker](src/components/PlayerFilterPicker.tsx), återanvänd av `SavedAggregatesCard`). **Inget "Table Name"-läge** (spel har inget namn). Grupper startar HELT ihopfällda (ingen default-expand; `initedSortRef` borttagen). **Bara spel med lagrade all-players-rader (`players`) listas** — poster skrivna FÖRE 2026-09-11 saknar fältet och filtreras bort (ingen HISTORY_Vx_RESET). Kort-innehåll (`GameCard`, ersätter `GameHistoryRow`): top-rad datum + `{correct}/{total} ({pct}%)` + chevron, sedan single → `Age · Assistance · Era`, multiplayer → `N players · Era` (`N = players.length`), båda + `Package · Sources`. **INGET namn / game-count / last-update** (det är Marathon-tabell-fält). Delete i leaderboard-modalen → `deleteGameHistoryEntry(id)` ([gameResults.ts](src/utils/gameResults.ts)) = **per-device/per-user** radering: historiken är redan namespacead per playerName, så motståndarnas egna devices behåller spelet, och ingår spelet i en marathon är serien (server) oberörd. HCP shield togs bort ur raderna 2026-05-18 och är inte återinförd där (HCP-systemet i övrigt är LIVE sedan 2026-08-28 — se "Dynamic HCP System").

   **HistoryEntry-shape (v5, 2026-08-26)**: utöver `correctAnswers/totalQuestions/avgResponseSeconds/age/assistance/eraFrom/eraTo` lagras `selectedExtraPackages: string[]` (tom = Generic; framtida theme-package-IDs när v1.1+) och **`sources: PlayedMediaSource[]`** — källorna som FAKTISKT serverades, i kanonisk ordning Spotify → YouTube → Hints. `GameCard` renderar meta-raden `Package: Generic · Sources: Spotify + YouTube + Hints`. `HISTORY_V5_RESET_KEY` wipe:ar pre-shape entries vid första load post-fix. Theme-packages + de per-källa-kategoriarrayerna överförs som URL-params från LobbyScreen:s `handleStartGame` (BÅDA host-path + non-host realtime-path) till quiz.tsx.

   **`players?: AggregateGamePlayer[]` (2026-09-11)**: ALLA spelares rader för spelet (samma form som en marathon-games `stats`), byggda ur den delade `buildGamePlayerStats()` i [quiz.tsx](app/quiz.tsx) — samma helper som matar marathon-bokföringens `contribution`, så de två kan aldrig glida isär. Ger (a) det tappbara final-leaderboard-kortet och (b) multiplayer-kortets spelarantal. OPTIONAL/additivt (`import type` från aggregateLeaderboard.ts — ingen värde-cykel, ingen HISTORY_Vx_RESET); Player history **filtrerar bort** poster som saknar fältet, så listan visar bara spel spelade efter detta. ⚠ Fältet fylls för ALLA non-guest/non-remote-spel (PtP/IndDev/single + marathon-spel) — marathon-spel ligger alltså redan ett-och-ett i per-spel-listan, ingen join mot aggregate-storen behövs.
...
- **Sekvens-auktoritet**: init-effekt (efter gameQuestionsRef) hämtar matchen; `question_ids` null + host → persistera lokala sekvensen EN gång; annars poll tills satt. `gameQuestions`-useMemo:n har remote-override överst: `remoteQuestionIds.map(id => ALL_QUESTIONS_MAP.get(id))` — host-resume och motståndare renderar identiskt. Render-gate ("Preparing 1vs1 match") blockerar allt spel-UI tills sekvens + resume-seed är klara. `totalQuestions` clampas mot sekvens-längden (katalog-skew → ids filtreras, saknas ALLA → "Update required"-Alert).
  ...
  - ⚠ **Allt slumpat frågeinnehåll MÅSTE vara deterministiskt i remote** (fix 2026-08-08). De två spelarna bygger sina ledtrådar och svarsalternativ LOKALT ur samma katalog utan sync-kanal — med `Math.random` fick de olika distraktorer, olika hints och olika ordning på exakt samma fråga (orättvis duell). Tre saker gör utfallet identiskt:
    1. **Seedad RNG** — [`createSeededRng(seed)`](src/utils/seededRandom.ts) (FNV-1a-hash → mulberry32). **`seedForSyncedQuestion(questionId)`** i [app/quiz.tsx](app/quiz.tsx) (f.d. `seedForRemoteQuestion`, generaliserad 2026-09-18) ger en seed för VARJE läge där frågan renderas på flera enheter samtidigt: `` `${remoteMatchId}:${questionId}` `` i remote (H2H) OCH `` `${roomCode}:${questionId}` `` i **Individual Devices** (varje enhet bygger sitt eget svarsblock lokalt → utan seed gav Math.random olika Name-alternativ per enhet, se 2026-09-18-fixen). `undefined` i PtP/Single Player (EN enhet, variation per runda önskvärd) + vid saknad nyckel (dev direkt-nav). roomCode delas av alla enheter i spelet och nygenereras per spel → identiska alternativ INOM spelet, variation MELLAN spel. Trådas till `selectHints(library, count, rng)`, `buildImageVariant(..., rng)` (som i sin tur skickar den till buildLetterGrid/buildNameOptions/buildFullNamesList), `ActorSelectBlock`s `optionsSeed` OCH **Spotify/Name** (`spotifyNameVariant`-effekten — seedad 2026-09-18; Spotify DJ är IndDev-only, så gissarnas enheter får identiska Name-alternativ via samma `roomCode:question.id`-seed; pool-bygget där är redan rena filter). **Seeda ALDRIG på något enhets-/spelarspecifikt** (playerId, timestamp, lokalt index) — det skulle just bryta cross-device-identiteten.
...
⚠ **PAKET-LÄGE KOLLAPSAR MIXERBOARDEN (2026-08-31).** När ett Host-paket är valt (`anyPackageActive`) renderar Lobby INTE längre den låsta 3×3-matrisen — den ersätts av **två aggregat-toggles: YouTube + Hints** (var och en styr HELA paketets material för den källan) plus **Spotify som komplement**. Nya boolean-fält `packageYoutubeEnabled` / `packageHintsEnabled` (default true) styr dem; de **synkas till non-host** via en SEPARAT targeted UPDATE i `setLobbySettings` (kolumner via **migration `0055_lobby_settings_package_toggles.sql`**, samma mönster som `remote_assistance` — INTE i `settingsToRow`; ej körd migration → console.warn + tolerant `?? true`). Tidigare skrevs de aldrig, så non-host visade YT som PÅ även när host kört enbart Spotify — det är fixat. en toggle är **disabled när paketet saknar material** för källan (`pkgHasYoutube`/`pkgHasHints` via `computePackageCoverage`; nuvarande paket = Music/YT/Spotify → Hints-toggeln disabled). quiz.tsx: `effectiveYoutubeCategories = packageActive ? (packageYoutubeEnabled ? coveredYtCats : []) : …`, `packagedImages` gatas på `packageHintsEnabled`. Era-låset (`packageEraLocked`) oförändrat. 3×3-matrisen visas fortsatt UTAN paket.
- **Spotify som ENDA källa** (2026-09-18, Peter — `allowSpotifyOnly`-grinden BORTTAGEN): Spotify får vara enda källan för VILKET paket som helst som faktiskt har Spotify-täckning. `pkgSpotifyOnlyOk` (LobbyScreen) = `anyPackageActive && pkgSpotifyActive` där `pkgSpotifyActive = pkgHasSpotify && spotifyEnabled && isSpotifyAvailable` → host får stänga av BÅDE YT och Hints (toggle-handlers + Start Game-validering släpper "at least one source"-kravet). Tomma-pool-skyddet ligger kvar i `pkgHasSpotify` (via `computePackageCoverage`) — ett paket utan Spotify-spår kan aldrig gå Spotify-only. quiz-poolen bygger Spotify-only automatiskt (ingen quiz-ändring krävdes). ⚠ `MusicPackage.allowSpotifyOnly` ([mockPurchasedPackages.ts](src/utils/mockPurchasedPackages.ts)) + `packagesAllowSpotifyOnly()` ([hostPackages.ts](src/utils/hostPackages.ts)) är nu OANVÄNDA/deprecated — behållna ifall en framtida grind behövs, men ingen kod läser dem.
...
## Free Premium launch promo (2026-08-09, server-grants sedan migration 0047)

QuizVibe lanseras med en kampanjperiod där **Premium är gratis**. Store visar då ett kort med rubriken **"Single month"** och underraden **"no auto-renewal"** plus en **Free**-knapp, och **79 kr-kortet visas inte alls**. När Peter stänger kampanjen tar betal-kortet över.

**TVÅ OBEROENDE KLOCKOR** — att hålla isär dem ÄR designen ([src/utils/promoPremium.ts](src/utils/promoPremium.ts)):

| | Vad | Styrs av |
|---|---|---|
| **Offer window** (global) | Om Free-kortet visas och om en gratismånad får tas | Peter, via `app_config.free_premium_promo` i Supabase + bakad `OFFER_BACKSTOP_UNTIL` |
| **Grant** (per KONTO, server-side) | Ett kontos Premium-månad — gratis eller voucher | Tabellen `premium_grants` (migration 0047), skrivs ENBART av RPC:erna `claim_free_month` / `redeem_voucher` |

- **Gratismånaden = EN gång per konto, evigt.** Unikt partiellt index `premium_grants_one_free_month` (`where source = 'free_month'`); ett andra försök ger `already_used`. Den gamla modellen med obegränsade förnyelser (device-lokal `CLAIM_KEY v2`) är BORTA — gamla lokala claims migreras inte.
- **Längden är alltid en HEL kalendermånad från trycket**: `now() + interval '1 month'` i RPC:n (Postgres klampar månadslängden: 31 jan → 28/29 feb). Den kortas ALDRIG av kampanjens slutdatum — claim dag 5 efter lansering ger en full månad, inte "resten av kampanjen".
- **Grandfathering**: att stänga erbjudandet återkallar INTE en pågående månad. Store byter till 79 kr-kortet, men den som har en månad igång behåller Premium tills DEN tar slut. Därför läser `hasActiveFreePremium()` MEDVETET bara grant-klockan, aldrig offer window. **Följd för planering**: sista möjliga Premium-dag = fönstrets slut + 1 månad.
- **Ingen stapling**: en månad (gratis eller voucher) kan bara aktiveras när INGEN premium är aktiv — servern svarar `already_active`. Förlängning efter gratismånaden sker bara via Peters individuella **voucher-koder** (`redeem_voucher`, engångskoder med `duration_months`).
- **Under pågående månad finns ingen knapp** — kortet visar grön `ACTIVE`-pill + "Free until {datum}".
- **Stänga kampanjen**: `update app_config set value = '{"enabled": false, ...}' where key = 'free_premium_promo'` i dashboarden. Slår igenom vid varje enhets nästa app-start — **ingen App Store-release**. `OFFER_BACKSTOP_UNTIL` (2026-12-31) är bara skyddsnätet för enheter som aldrig når Supabase.
- **Lokal spegel** (`MIRROR_KEY`, `{ premiumUntil, freeMonthUsed, owner }`) fylls av `refreshPromoGrants()` (RPC `get_promo_premium_status`) vid app-start, SIGNED_IN och efter claim/redeem. Den är **ägarstämplad** (owner = playerName lowercased) så ett annat konto på samma telefon aldrig ärver månaden; `readOwnedMirror` returnerar null vid mismatch. Nätverksfel lämnar spegeln orörd (fail-open).
- **Den betalda spegeln rensas vid SIGNED_IN** i [_layout.tsx](app/_layout.tsx) (`clearPremiumSubscription()` kedjat FÖRE `configurePurchases(uid)`) så ett nytt/icke-premium-konto aldrig ärver förra kontots stale `true`. Grant-spegeln överlever. Rensa ALDRIG grant-spegeln vid logout.
...
- **HCP visas som SKÖLD** på spelarkortet (2026-08-28) — `hcp`/`hcpNotDefined`-props → `<HCPShield size={40}>` bredvid avataren (viewerns egen rad = intjänat HCP via `selfHcp`, andra spelare = deras synkade `player.hcp` via lobby_players.hcp/0042, fallback 99; gäst = "Guest"). Se "Dynamic HCP System" → "Lobby cross-player HCP-sync". Den tidigare TEXT-HCP-badgen (borttagen 2026-05-21) kom aldrig tillbaka.
...
⚠ Det här är BAS-helpers. Den LIVE spelar-progressionsmotorn (stackad delta: fönstersteg × tier-faktor × Game-Era-bredd × assistance-riktning, ±10-capp, decay, display) bor i [hcpEngine.ts](src/utils/hcpEngine.ts) + [hcpProgress.ts](src/utils/hcpProgress.ts) — se "Dynamic HCP System" nedan.
...
- **`getGuestHcpFromClosestAge(guestAge, registeredPlayers)`** — guest:s dolda HCP-värde:
  ...
  - Visuellt visar guest-kort ALLTID **"Guest"**-sköld (utan siffra, `HCPShield notDefined` — prop-namnet är stale, den rendrerade texten är "Guest"). ⚠ `getGuestHcpFromClosestAge` är i praktiken UNUSED — ingen kod anropar den; en joinande gäst tilldelas alltså inget bakgrunds-HCP (Peter-beslut 2026-09-12 "let it be as it is"). Ett guest-hostat spel driver difficulty via det hårdkodade HCP 99 i quiz.tsx, inte via denna helper.

## Dynamic HCP System (LIVE sedan 2026-08-28)

⚠ **UPPDATERAD 2026-09-16 — snabbare rörelse + Game-Era/assistance-skalning.** Delta-motorns konstanter är omgjorda så HCP rör sig mer och tidigare, och spelets **Game Era** + **assistance-nivå** skalar nu deltat. Ändringar: fönster **20 → 10**; trösklar Full 3/7, Standard 3/6, Minimal 2/5; tier-faktorer **1.0/0.7/0.4** (var 0.75/0.5/0.25); den gamla per-nivå×tier-`WORST_DELTA_CAP` **borttagen**, ersatt av ett symmetriskt `MAX_GAME_DELTA = 10` (inget spel rör HCP mer än ±10); NYA `ASSIST_DIRECTION_MULT` (belönar hårdare inställningar) + `eraWidthMultiplier` (bredare era = mer). Datamodell, start (99), decay, kategorier, item-filter, sync och sköldar är OFÖRÄNDRADE. Se "Peters designbeslut" + "Ren motor" nedan för aktuella värden — äldre siffror i detta stycke är historik.

⚠ **UPPDATERAD 2026-08-31 — HCP är nu 4 värden PER REGION SCOPE.** Delar av beskrivningen nedan (skriven för den enkelvärdes-modellen) är historik; mekaniken (sliding-window, decay, filter) körs nu PER KATEGORI och nyckeln bär en region-segment (steg-storleks-mekaniken ändrades dessutom 2026-09-16, se ovan). Det som ändrades:
- **HCP per spelare**: Total + Music + Film (Sport borttaget ur modellen 2026-09; katalogen parkerad). **Total = SNITTET av de två live kategorierna** (härlett, aldrig lagrat). `HcpProgress` i [hcpEngine.ts](src/utils/hcpEngine.ts) är nu `{ categories: Record<MainCategory, CategoryProgress> }`; varje `CategoryProgress` har eget `hcp`+`windows`+`lastPlayedISO`. `applyGameResult(progress, category, level, answers, nowISO)`, `totalHcp`, `resolveDisplayTotalHcp`, `resolveDisplayCategoryHcp` (alias `resolveDisplayHcp` behållet). 53 tester.
...
**Peters designbeslut (2026-08-28, delta-modell omarbetad 2026-09-12, snabbare + Game-Era/assistance-skalning 2026-09-16):** alla nya spelare startar på **HCP 99** (assistance/ålder-baserat start avfärdat); **fullt 10-svars-fönster** innan första steget (~2–4 spel), sedan kontinuerligt glidande. Deltan är INTE ±1 — den **stackas multiplikativt**: `råsteg × TIER_FACTOR(HCP före) × eraWidthMultiplier(span) × ASSIST_DIRECTION_MULT[nivå][riktning]`, och klampas symmetriskt till **±MAX_GAME_DELTA (10)**. Se nedan.

**Ren motor** — [src/utils/hcpEngine.ts](src/utils/hcpEngine.ts) (INGA React/AsyncStorage-beroenden; Date injiceras → deterministisk; enhetstestad i [backend/content/test/hcpEngine.test.ts](backend/content/test/hcpEngine.test.ts), 61 tester):
- `HcpProgress = { categories: Record<MainCategory, CategoryProgress> }`; `CategoryProgress = { hcp, windows: Record<AssistanceLevel, boolean[]>, lastPlayedISO }`. `hcp` lagras som FLYTTAL (delta ger decimal-delar, decay 0,5); DISPLAY avrundas ALLTID UPPÅT (§1.2.3) via `displayHcp` (ceil + clamp [1,99]).
- `HCP_START = 99`, `HCP_WINDOW_SIZE = 10`. Trösklar (fönstersumma S, på 10-fönstret): Full S≥7 → nedåt (bättre) / ≤3 → uppåt (sämre); Standard ≥6 / ≤3; Minimal ≥5 / ≤2.
- **`evaluateWindow(win, level)` → RÅTT signerat stegantal** (okapat): `−(S − upper + 1)` när S≥upper, `+(lower − S + 1)` när S≤lower, annars 0. Ex FULL (3/7): 10/10→−4, 7/10→−1, 3/10→+1, 0/10→+4. Kräver FULLT fönster (10) → ny spelares HCP stabilt ~2–4 spel.
- **`hcpTier(hcp)` / `hcpTierFactor(hcp)`** — steg-faktor ur HCP:t FÖRE spelet: **>60 → ×1.0, 30–60 → ×0.7, 1–29 → ×0.4** (60.0/30.0 → mid). Gäller BÅDA riktningar; fortsatt långsammast nära elit så eliten sitter kvar.
- **`ASSIST_DIRECTION_MULT[nivå][improve|worsen]`** — belönar hårdare inställningar: **improve** Full/Std/Min ×1.0/1.2/1.3, **worsen** ×1.0/0.8/0.6. Mindre hjälp ⇒ bra spel räknas mer, dåligt mindre (`raw < 0` = improve). Up/down-asymmetrin bärs numera ENBART av denna faktor.
- **`eraWidthMultiplier(eraYears)`** = `span/15` klampad **[0.5, 3.0]** (15-årsera ×1.0, 30 ×2.0, ≥45 ×3.0; smalaste 10-årsera ×0.67). Bredare Game Era = svårare att placera = mer progression. `span = eraTo − eraFrom` (skickas från quiz-slutet; ogiltigt/saknat → 15 = neutralt).
- **`MAX_GAME_DELTA = 10.0`** — ersätter den gamla per-nivå×tier-`WORST_DELTA_CAP`. Slutdeltan klampas symmetriskt till ±10 (stackade multiplikatorer kan ge stora råa deltan på breda eror, men inget spel rör HCP mer än 10 poäng, båda hållen).
- `applyGameResult(progress, category, level, answers, eraYears, nowISO)` = lägg svar i nivåns fönster → `råsteg × tier-faktor × era-faktor × assist-riktning` → klampa ±MAX_GAME_DELTA → klampa HCP [1,99] → stämpla lastPlayedISO. `applyInactivityDecay` (§2.4: **+`WEEKLY_DECAY` = 0.5** per hel 7-dagarsperiod — höjt från 0.25 av Peter 2026-09-22, så en månads uppehåll kostar ~2 HCP i stället för ~1; flyttar lastPlayed fram med HELA perioder, ej till now, så vecko-resten bevaras + ingen dubbelräkning).
- `resolveDisplayHcp(storedHcp)` → `storedHcp ?? 99`, ceil:at. Sköldarnas ENDA display-källa.

**Persistens** — [src/utils/hcpProgress.ts](src/utils/hcpProgress.ts) (epochLedger/hostQuestionHistory-mönstret): nyckel `@quizvibe/hcpProgress/v2/<region>/<playerName.toLowerCase()>` (per region scope), gäst-fallback = sessions-lokal `sessionProgress` per region. `recordSelfGameResult(region, level, answersByCategory, eraYears)` (inloggad: decay → game result → save → **speglar profile.hcp**), `recordGameResultForName(name, region, level, answersByCategory, eraYears)` (PtP-medspelare per namn, INGEN profil-spegling), `loadOwnCategoryHcp(region)` (per-kategori-HCP → item-filtret), `refreshOwnHcpDecay(region)` (decay vid app-open så skölden reflekterar inaktivitet — anropas i ProfileScreen mount), `clearOwnHcpProgress`. `coerceCategory` trimmar lagrade fönster till senaste `HCP_WINDOW_SIZE` vid läsning så befintliga spelares gamla 20-långa fönster inte avfyrar ett överstort delta på nästa spel (självläkande, ingen migration).
...
- **Game Era**: spelets span (`eraTo − eraFrom`, fallback 15) beräknas i effekten och skickas till `recordSelfGameResult`/`recordGameResultForName` → skalar deltat via `eraWidthMultiplier` (bredare era = mer intjänat).
...
**§4.1 Item-HCP-frågefilter (LIVE sedan 2026-08-28; tiered golv sedan 2026-09-12):** varje fråga bär `itemHcp` = katalogens `probability` (0–100), exporterat på klienten (`ExportedMusicQuestion`/`ImageQuizQuestion` → `QuizQuestion`; regenerera med `npm run export-music-questions` + `export-image-questions`). ⚠ **Peters filter-regel (2026-09-12) är ett ENSIDIGT golv per spelar-HCP-nivå — INTE längre `itemHcp >= playerHcp` och INTE det tvåsidiga bandet `[HCP−20, HCP+80]`.** Övre kanten är alltid 100 (inget tak); bara golvet varierar:

| Spelar-HCP | itemHcp-golv |
|---|---|
| ≥ 80 | ≥ 10 |
| 60–79 | ≥ 8 |
| 40–59 | ≥ 6 |
...
Nettoeffekt: golvet sänks stegvis när HCP tjänas ner, så de mest obskyra items (lägst probability) låses gradvis upp. Nybörjare (HCP ≥ 80) utesluts bara från de allra obskyraste (probability < 10); experter (HCP < 20) når hela poolen.
- **Ren helper** `hcpRecognitionLowerBound(playerHcp)` + `HCP_RECOGNITION_TIERS` i [hcpEngine.ts](src/utils/hcpEngine.ts) (enhetstestade) ger golvet; `filterByItemHcp(pool, playerHcp, minCount)` behåller items med `itemHcp >= golv` (saknat `itemHcp` = 100 → alltid kvar). `minCount` är en liten-katalog-tröskel (pool ≤ minCount → filtreras inte); tomt band → hela poolen (skyddsnät, i praktiken onåbart då golvet ≤ 10). Den gamla `HCP_FILTER_STEP`/variety-floor-vidgningen och `HCP_BAND_BELOW/ABOVE` är BORTTAGNA.
- **Tillämpas** i [quiz.tsx](app/quiz.tsx) `gameQuestions` via `applyItemHcp(pool)` på de tre pool-erna (spotify/pureYoutube/image), PER KATEGORI via `filterPoolByCategoryHcp` (Music-fråga mot Music-HCP osv.). `HCP_FILTER_MIN_POOL = 30`. Gäller **Single Player + Pass-the-Phone** (INKL. guest-hostade sådana sedan 2026-09-12); **IndDev** (delad host-sekvens) och **remote** (server-sekvens) filtreras INTE. Källan är `hcpSource = isGuestHostGame ? null : regionHcp` — vanliga spel läser DENNA enhets per-kategori-HCP via `loadOwnCategoryHcp(region)`, medan **guest-hostade spel tvingar null → HCP 99** (ny-spelar-beteende) för alla kategorier. Det gäller även en inloggad user som hostar som gäst: deras riktiga HCP ignoreras (låst trial), skölden visar ändå "Guest" och inget HCP sparas efteråt (§5-motorn grundar fortsatt inget för guest-hostade spel).
- ⚠ **Bootstrap-notis:** eftersom `probability` toppar runt 80–90 och det högsta golvet är 10, är filtret milt i topp — skillnaden mellan nivåerna är att de obskyraste ~0–10-items gradvis släpps in. Riktig per-generation/per-item-difficulty (§1.2) ersätter probability-bootstrappen senare.
...
⚠ **Join-POPUPEN hålls också tillbaka för inbjudna/friends under grace-fönstret (Peter 2026-09-24).** Samma konvergens-glapp som den röda blinken: en inbjuden spelare (t.ex. som just slog på Spotify i join-gaten) sågs först som icke-auto-approvable, popupen köades direkt och försvann en sekund senare när auto-approven landade. Watchern hoppar nu över köningen när `isAutoApproveCandidate(p)` (namn-match mot friends ∪ pendingFriends ∪ invitedNames, MEDVETET utan type-check) OCH id:t ligger i `joinGraceActiveRef` (skrivs synkront av grace-effekten, släpps av dess timer FÖRE `setJoinGraceIds`; `joinGraceIds` står i watcherns deps så den körs om när fönstret stängs). Auto-approve-grenen påverkas inte. Främlingar får popupen direkt; en inbjuden som genuint blockeras (t.ex. utan Spotify) får den efter ~3 s.
...
- **Färre rundor än spelare → Spotify utgår helt** (inget halvt varv). Spotify som ENDA källa: `handleStartGame` i LobbyScreen **blockerar** start om rundorna inte är en multipel av spelarantalet (Alert "Uneven Spotify DJ turns" som föreslår giltiga rundantal, t.ex. 6/12/18 för 3 spelare; Peter 2026-09-24) — DJ:n svarar aldrig på sin egen fråga, så en extra DJ-tur = en svarschans mindre. quiz.tsx:s utfyllnadsgren (Spotify tar resten när ingen annan källa finns) är kvar enbart som skyddsnät.
...
**Spelbarhets-gate (Peter 2026-09-03, ersätter 2026-06-06 + 2026-08-27):** `meetsHintsThreshold(library)` i [hintsText.ts](src/utils/hintsText.ts) — ett hints-item når spelaren BARA om **(1) `library.hints.length >= MIN_RAW_HINTS` (nu `8`) OCH (2) `hasCountryFlag(library.nationality)`** (en riktig landsflagga i FLAG_MAP, inte den grå `🏳️`-platshållaren för `'unknown'`/omappat land). Samma funktion driver BÅDE `export-image-questions.ts` (vad som bakas in i `quizImageQuestions.ts`) och quiz.tsx:s runtime-filter av `IMAGE_SEED_QUESTIONS` — ändra regeln på ETT ställe. Flagg-kravet läser den **överridade** nationaliteten: `NATIONALITY_OVERRIDES` i [hintsData.ts](src/utils/hintsData.ts) patchar `'unknown'` → rätt land INNAN `HINTS_LIBRARY` byggs, så items med korrigerbar nationalitet (t.ex. `bts`, `imagine-dragons`, `r-e-m`, `the-beach-boys`) behåller sin flagga i stället för att gallras bort — lägg nya sådana där. **Historik:** 2026-08-27-gaten hade en alternativ väg in (`≥5` grupperade topp-nivå-bullets via `countPlayableEntries`/`MIN_RENDER_ENTRIES`) som släppte in tunna items som `rolandz` (5 lösa fakta + grå flagga). Den vägen är BORTTAGEN ur gaten; `countPlayableEntries`/`MIN_RENDER_ENTRIES`/`countRenderEntries` finns kvar exporterade men ingår inte längre. `MIN_HINTS_REQUIRED` (den gamla konstanten i quiz.tsx) finns inte längre — allt går via `MIN_RAW_HINTS` i hintsText.ts.
...
⚠ **`export-image-questions.ts` gatade på `.webp`** och höll därför tyst nytt hints-innehåll utanför poolen (de 6 golfarna + Håkan Mild och Jonas Thern). Sedan person-bilderna parkerades juridiskt renderar en image-fråga bara flagga + ledtrådar och rör aldrig bildfilen (`getQuizImage` är utkommenterad i quiz.tsx), så webp-beroendet är helt borta — exporten (`listHintItemIds`) väljer numera items via `meetsHintsThreshold` (se "Spelbarhets-gate": ≥8 hints + riktig flagga). Ett nytt hints-item behöver alltså INGEN bild.
...
- Alla lobby-guards (toggle-on Check 1–2, approve Check 3–4, handleStartGame-guard) kvar med samma mekanik, omformulerade utan "Premium"/"account". **Pre-join attest-gate (2026-09-24)**: `checkSpotifyLobbyGate(code)` i [app/index.tsx](app/index.tsx) körs i `proceedAcceptInvite` (före addFriend/removeInvite) och `handleJoinWithCode` — lobbyn har `spotifyEnabled` + inloggad profil utan `spotifyAppConfirmed` → overlay i JoinModal (vanlig View, ej andra Modal) "This lobby uses Spotify" med samma "I have Spotify App on this device"-switch som lobbyn + Cancel. Switch ON → Alert "Update your profile?" OK/Cancel; OK = install-check (`not-found` → blockera) + `saveProfile({spotifyAppConfirmed: true})`, så code-only-joinen publicerar `spotifyConnected` och host:s auto-approve passerar utan host-ingrepp. Cancel = kvar i Join-formuläret (inviten kvar). Guest-join berörs inte (auto-approvas aldrig).
...
**Döda / blockerade klipp — pre-game först, popup som skyddsnät (2026-09-23, ERSÄTTER 2026-06-03-vägen med "Video unavailable" + 0 pts).** Bakgrund: `drake-one-dance` (re-upload av "Billion Stars") gav "Video unavailable" i TestFlight (SE) fast Data API, oEmbed och CI:s embed-audit alla sa spelbart — claim-baserade embed-block per land syns BARA vid riktig uppspelning från landet. (1) **Pre-game (primärt)**: `validateSequenceClips` ([quiz.tsx](app/quiz.tsx)) kollar spelfönstret + `CLIP_PROBE_RESERVE` innan Play och byter döda frågor mot reserver (`swapDeadYtItems`) — utbytt fråga räknas som vanligt. `checkYoutubeClipsAlive` ([youtubeLiveness.ts](src/utils/youtubeLiveness.ts)) kör oEmbed PARALLELLT med `serverCheck` = Edge Function **`check-clips`** ([supabase/functions/check-clips](supabase/functions/check-clips/index.ts), Verify JWT AV, secret `YOUTUBE_API_KEY`): Data API (raderat/privat/embeddable=false/åldersgräns/regionRestriction utan SE) ∪ **blocklistan** `clip_playback_errors` (migration **0056**; bara `video_not_found`/`embed_not_allowed`, 30 dagar, ej `cleared`). serverCheck kan bara LÄGGA TILL döda ids, aldrig säga levande; fail-open. (2) **In-game (skyddsnät)**: `handleYoutubeError` → `handleClipUnavailable` rapporterar felet (`reportClipError` i [clipErrorReport.ts](src/utils/clipErrorReport.ts) → blocklistan, så NÄSTA spel byter ut klippet före Play), fryser timer + scoring-latch och visar Alert **"Clip unavailable / Sorry but this clip has been removed for your region"**. Host/single/PtP byter frågan på SAMMA index (`pickReplacementQuestion` → `lockedSequenceRef`) och går till Get Ready utan PtP-rotation (`handleAdvanceToNextRound(idx, { keepPlayer: true })`); trasiga frågan får ingen poängrad. IndDev: non-host skickar `clip_unavailable` till host; host broadcastar `question_advance` med samma index + **`swap_question_id`** (ingår i dedupe-nyckeln `lastHandledAdvanceRef` — utan den droppas swap-advancen). Remote 1v1 (server-låst sekvens): frågan HOPPAS ÖVER via `upsertAnswer({ skipped: true })` (migration **0057**; forfeit-RPC:n ignorerar skippade rader, resume-seeden hoppar dem). Ingen reserv → hoppa till nästa plats utan poäng. ⚠ Försök INTE byta till nästa klipp på itemet mitt i frågan (gamla C1-fallbacken) — Peter vill hela frågan bytt. `failedClipVideoIds` används bara av pre-game-kollen. (3) **Nightly auto-PR**: rött `validate` → jobbet `autofix-pr` kör autofix --apply + export och öppnar PR `bot/clip-fix` med review-request till peterlomaco (tabell via [clip-fix-pr-body.ts](backend/scripts/clip-fix-pr-body.ts), klickbara länkar + ⚠ för inofficiell kanal / år i titeln). Merge → [ota-on-clip-fix-merge.yml](.github/workflows/ota-on-clip-fix-merge.yml) publicerar OTA till `preview` (kräver secret `EXPO_TOKEN`; byt till production vid launch).
...
**Valbara countdown-röster — röstpack (`expo-audio`, 2026-09-06)**: spelaren väljer countdown-röst i Profile → "Countdown voice". Ett röstpack är en handfull förinspelade mp3-klipp som ERSÄTTER `expo-speech` i CountdownIntro; `DEFAULT_VOICE_ID = 'default'` behåller system-TTS-rösten (och är fallback om ett pack inte laddas).
- **Vokabulär = fast litet set**: bara tokens `1/2/3` (siffrorna som talas idag, `voiceFrom` default 3) + `when`/`who`. `4`/`5` bundlas som reserv. Manifest + assets: [voicePacks.ts](src/utils/voicePacks.ts) (`VOICE_PACKS`, `VOICE_OPTIONS`, `getVoicePack`) + `assets/voice-packs/<id>/{1,2,3,4,5,when,who}.mp3`. Lägg till pack = ny mapp + post i `VOICE_PACKS`; pickern följer med.
- **Persistens**: `ProfileData.voice?: string` — **AsyncStorage-only** (ingen DB-kolumn, samma mönster som `parentControlEnabled`; merge:as i `loadProfile`). Sparas via `handleSave('defaults')`.
- **Device-lokalt**: [quiz.tsx](app/quiz.tsx) läser `getCachedProfile()?.voice ?? DEFAULT_VOICE_ID` vid countdown-render och skickar `voice`-prop — INGEN URL-param (audio-preferens per enhet, som "Audio this device"). Non-hosts är silent ändå; `silent`-grinden ligger kvar. Gäst/ohydrerad → Default.
- **CountdownIntro**: preloadar en `AudioPlayer` per token vid mount (bara host + bara pack); tick/start spelar `player.seekTo(0); player.play()` när pack finns, annars den oförändrade `Speech.speak`-vägen. Pre-warm hoppas över för pack. [voicePlayback.ts](src/utils/voicePlayback.ts) sätter audio-mode (`playsInSilentMode` + `interruptionMode: 'mixWithOthers'` så WebView-ljuden inte avbryts) + driver Profile-pickerns ▶-preview.
- ⚠ **`expo-audio` = native modul → kräver ny dev/TestFlight-build (ingen OTA, ej Expo Go).** Verifiera på enhet: röst spelas, timing känns rätt (ev. tuna `VOICE_LEAD_MS`), Default faller tillbaka på system-TTS, non-host tyst, klipp mixar med YT/ambient.
- **6 pack (Peter 2026-09-07, energiska/positiva först)**: `hype` (Hype), `cheer` (Cheer), `bright` (Bright), `announcer` (Announcer), `coach` (Coach), `deep` (Deep, dramatisk kontrast). Ordningen i `VOICE_PACKS` = ordningen i pickern (Default överst).
- ⚠ **Klippen i `assets/voice-packs/` är PLATSHÅLLARE** (Windows-TTS David/Zira + SSML pitch/rate/emphasis — begränsad "energi", robotiska). Kör [backend/scripts/generate-voice-packs.ts](backend/scripts/generate-voice-packs.ts) med en cloud-TTS-nyckel (OpenAI default, `OPENAI_API_KEY`) för de RIKTIGA rösterna — varje pack är mappat till en OpenAI-röst + energisk `instructions`-stil (hype→shimmer, cheer→coral, bright→nova, announcer→fable, coach→ash, deep→onyx). SAMMA filnamn, inga kodändringar.
...
- ⚠ **Både ritningen OCH sekvensen ankras till FÖRSTA rAF-framen, aldrig till effekt-/mount-tiden (Peter 2026-09-22).** Celebrationens egen mount är tung: `SparkLayer` monterar ~100 skugg-tunga `Animated.View` (iOS-skuggor är dyra) och `SparklerSound`-WebView:n startar i samma render-burst → JS-tråden blockeras ~1 s PRECIS när ritningen ska börja. Tidigare läste SparkleDrawQ wall-clock (`Date.now() − startedAt`) med `startedAt` satt SYNKRONT i effekten, så under blocket rusade klockan iväg och bågen "hoppade" in ~85 % ritad — spelaren såg aldrig elden forma Q:t. Nu sätter SparkleDrawQ `startedAt` + startar den native-drivna gnistanimationen INNE i sin första `requestAnimationFrame`-callback (en rAF fyrar först när tråden är fri och en frame faktiskt målas), och FinalCelebrations sekvens-effekt startar markIn/glöd/pokal/konfetti/`startedAtRef`/watchdog i EN egen `requestAnimationFrame`. Båda rAF:erna fyrar på samma första post-jank-frame → ritningen syns 0→100 % OCH pokal/konfetti/ljud delar samma anchor och landar på Q:ts avslut. `startedAtRef` (som `SparklerSound` läser) sätts i den rAF:en, så `setPlaySound(true)` MÅSTE också ligga där inne, annars monteras ljudet med `startedAt=0`. Watchdogen schemaläggs numera OCKSÅ i rAF:en så dess `total`-marginal mäts från den verkliga starten (annars kunde ett långt block skjuta starten förbi marginalen och forcera `highlights` i förtid). **Sätt aldrig tillbaka draw-/sekvens-starten till synkron effekt-tid.** Reduce Motion är dessutom förresolverad under `ready`-beaten (`reduceMotionRef`) så sekvensen läser den synkront utan ett `await` som annars skulle skjuta konfettin efter ritningen.
...
**Gäller/gäller inte:**
- **Pass-the-Phone OCH Individual Devices**: ⚠ **HAR re-match sedan 2026-08-26** (Peter) — undantaget 2026-08-25 är upphävt. Två regler bär det:

  **(a) Behörighet — ALLA lokala multiplayer-spel (PtP OCH IndDev, Peter 2026-09-18).** `rematchBlocked` kräver att ALLA deltagare är registrerade QuizVibe-users: `!isLocalSoloGame && turnOrder.some(p => p.type === 'guest')`. En enda gäst — värd-tillagd via "+ Add Player" i PtP ELLER **självansluten anon på egen enhet i IndDev** — och frågan visas inte alls (slutskärmen ser ut som före 2026-08-26). Det är detta som upphäver den gamla invändningen: spel med gäster är helt enkelt inte behöriga, i stället för att approvals byggs runt dem. Bonus: alla deltagare har då `user_id`, så Competition-serien blir **server-sparad** i stället för lokal-bara.
  ⚠ **Gäst-villkoret är INTE längre PtP-only** (var det t.o.m. 2026-09-18). Den gamla koden antog att IndDev aldrig har gäster, men självanslutna gäster behåller sin egen enhet + anon-session och FÅR spela IndDev (policy 2026-08-06) — så en IndDev-`turnOrder` kan bära `type === 'guest'`, och re-match erbjöds felaktigt. Alla tre gaterna (`rematchBlocked`, `rematchImpossibleForGame`, `rematchUnavailableNote`) har därför tappat sitt `gameMode === 'pass-the-phone'`-villkor.
  ...
  ⚠ `isLocalSoloGame` MÅSTE stå först — solo bär oftast `gameMode: 'pass-the-phone'` (profil-defaulten). Samma fälla som buggen 2026-08-26. (Solo har ingen gäst-joiner, så gäst-villkoret rör det ändå inte.)
...
⚠ **Ordningen är betydelsebärande.** Gäst-fallet testas FÖRE aktiv-fallet: ett multiplayer-spel med gäster kan falla på BÅDA (en gäst kan koppla ner och därmed sakna "aktiv" enhet), och då är gäst-skälet det sanna — "not all players are active" hade fått det att låta som att någon gick därifrån. Alla texter har explicit `
...
⚠ **UI-copyn heter "Marathon table" / "Marathon Score"** (Peter 2026-08-26; omdöpt från "Competition Leaderboard" / "Competition Score" 2026-08-29). Koden, filnamnen och HELA databasen (`aggregate_leaderboards`, `create_aggregate_leaderboard`, `aggregateLabel`, `SavedAggregatesCard`, `Competition*`-lagret …) heter fortsatt `aggregate*`/`Competition*` — migration 0037 är körd, och ett namnbyte där hade krävt en ny migration utan att ge användaren något. Sektionerna nedan använder kod-namnet. **Byt inte kod-identifierare för att "matcha" copyn**, och lägg inte till nya användarsynliga strängar med orden "Aggregate" eller "Competition" — etiketten byggs av `aggregateLabel()`. ⚠ **Detsamma gäller SKÄRMEN: Home-knappen läser "Marathon" ([CompetitionsButton.tsx](src/components/CompetitionsButton.tsx)) och skärmtiteln är "Marathons" ([CompetitionsScreen.tsx](src/screens/CompetitionsScreen.tsx)) — kalla den "Marathon-skärmen (route `/competitions`)", ALDRIG en "Competitions-skärm".** Route-pathen `/competitions` + `CompetitionsScreen`/`CompetitionsButton` är interna kod-namn, aldrig synliga för spelaren.
...
- **Profile**: [SavedAggregatesCard](src/components/SavedAggregatesCard.tsx) renderas inuti Player history, ovanför månadsgrupperna (`<SavedAggregatesCard showRematch />` sedan 2026-09-10 — se "Marathon-listan (Profile)" nedan). Självgatande — inget sparat eller anon-session → `null`.
...
### Marathon-listan (Profile) — kort-layout, sortering, filter, re-match (2026-09-10, Date-sort tre-nivå + shared filter 2026-09-11)

[SavedAggregatesCard](src/components/SavedAggregatesCard.tsx) på Profile omstrukturerad. Grupperingen delas med per-spel-historiken via [historyGrouping.ts](src/utils/historyGrouping.ts) (`groupHistory` + `groupHistoryByMonthDateForm`, rena + vitest-testade).

- **Rad-meta**: `N games` (+ `· N players` ENBART när > 1 deltagare — solo visar bara antal spel). Den gamla `aggregateLabel(...)`-etiketten ("Marathon Score"/"Marathon table") och deltagar-namnlistan är BORTA ur raden (`aggregateLabel` lever kvar för namn-prompten). Kort-containerns rubrik "Marathon tables" är orörd.
- **Sist spelad-datum** i radens övre högra hörn med prefix `Last update:` (`item.updatedAt ?? item.createdAt`, `toLocaleDateString('en', …)`), ovanför chevron:en.
- **Tre sorteringar** (`SortMode = 'host' | 'date' | 'name'`): **Host Name** (DIN egen host överst, sedan alfabetiskt, "Unknown host" sist), **Date** och **Table Name** (alfabetiskt på tabellnamnet). Self-first drivs av `getCurrentUserId()` ([auth.ts](src/utils/auth.ts)) → jämförs mot `item.createdBy` via `groupHistory`:s `getHostUserId`-accessor + `selfUserId`-param (host-läge sätter `isSelf` på hinken).
  - ⚠ **Date-sorten är TRE-NIVÅ** (2026-09-11, commit `bcdce43`): månad → datum → spelform, via `groupHistoryByMonthDateForm` (månader OCH datum nyast först, "Unknown date" sist på båda nivåerna; `CollapsibleGroup` level 1 = månad, level 3 = datum, level 2 = spelform — `expandedL1`/`expandedDates`/`expandedForms`). Host/Name-lägena är fortsatt TVÅ-nivå (`groupHistory`, l1 → spelform, `groups=[]` i date-läget och `dateGroups=[]` i host/name). Flash-guiden grenar på `isDateSort` och öppnar månad/datum/form-nycklarna därefter. Samma strukturen återanvänds av per-spel-historiken (se "Player history"-punkt 5).
- **Grupperna startar HELT ihopfällda** varje gång Player history öppnas OCH efter sort-byte (default-expand-effekten borttagen). Flash-guiden (`focusIds`, /competitions) är orörd.
- **Per-user delete (migration 0052)**: röd **Delete** uppe till höger i detalj-modalen → `dismissAggregateLeaderboard(id)` sätter bara MIN deltagar-rads `dismissed` (övriga behåller serien); härlett "locked" gör serien permanent olåsbar för re-match. Fokus-effekten filtrerar bort serier jag dismissat (`participants.find(userId===uid)?.dismissed`). [SavedAggregate](src/utils/aggregateLeaderboards.ts).participants bär `dismissed?`; `rowToSaved` mappar `p.dismissed ?? false` (tolerant tills 0052 körts).
- **Filtrera på QuizVibe-friend** via [PlayerFilterPicker](src/components/PlayerFilterPicker.tsx) — bottom-sheet-picker (mönster som Lobbyns Share invite) **utbruten 2026-09-11 till delad komponent** (samma filter i per-spel-historiken). `loadFriends()` → lista med kryssruta per friend + "Add" på "All players"-raden. Multi-select → **AND-filter**: bara marathons där ALLA ikryssade deltog (matchning på `playerName` case-insensitivt mot deltagarnas snapshot-namn — friends bär inget `userId`). Tom träff → "No marathons where all selected players took part." Kontrollerad: parent äger `appliedFriendIds`, picker äger pending/open-state.

**Re-match från Profile-kortet = TVÅ-FAS-flödet (samma som /competitions), inte en en-tapp.** `PlayerHistorySection` renderar `<SavedAggregatesCard showRematch />`, så detalj-modalen kör [CompetitionRematchActions](src/components/CompetitionRematchActions.tsx): host **"Send Re-match invitation"** (f.d. "Re-match", omdöpt 2026-09-10) → `createRematchRequest` → "Waiting for N of M players to accept" → när alla accepterat **"Yes – start re-match"** → lobbyn skapas och alla dras in samtidigt. Solo → "Replay" (direkt single-lobby). ⚠ **Bygg INTE en en-tapp-variant** som skapar lobbyn direkt + "Invitations sent"-popup — det byggdes 2026-09-10 och revs samma dag; host ska inte in i en lobby förrän alla accepterat.

⚠ **Competition re-match-lobbyn har INGEN 5-minuters self-destruct längre** (borttagen 2026-09-10). I två-fas-flödet accepterar alla FÖRE lobbyn skapas, så en timeout fyrar aldrig meningsfullt. `COMPETITION_REMATCH_TTL_MS` + `allRematchPresentRef` + timer-effekten är borta ur [LobbyScreen.tsx](src/screens/LobbyScreen.tsx). Den FAKTISKA spärren är kvar: `handleStartGame`:s lineup-lock-guard (Start blockeras tills hela `rematchExpectedIds`-uppsättningen är på plats, Alert "Waiting for players") + "Delete this Game Lobby" som manuell utväg. `rematchExpectedIds` + dess populate-effekt behålls (guarden + kapacitetsvyn använder den).

**Marathon-namn på inbjudan (migration `0051_waiting_invites_competition_name.sql`, applicerad i staging + prod 2026-09-10):** `waiting_invites` fick en nullable `competition_name`-kolumn. `startCompetitionRematch` skickar `competitionName: saved.name` på varje `addInvite` (fas 2, cross-device join-fallback), och invite-kortet i [app/index.tsx](app/index.tsx) visar då badgen **"Marathon table"** (i stället för den härledda `resolveInviteGameMode`-etiketten Single/Multiplayer/H2H) + tabellnamnet under Room Code. ⚠ Namnet kan INTE härledas cross-device via `roomCode` — room→serie-kopplingen bor bara på hostens enhet — därför bärs det på inbjudan. ⚠ `addInvite` lägger `competition_name` i INSERT-payloaden ENBART när den är satt (marathon-invite), så vanliga Share-invites är byte-identiska och en icke-applicerad 0051 bryter aldrig det ordinarie flödet (okänd kolumn i en INSERT failar HELA raden). `select('*')` + `row.competition_name ?? undefined` tål saknad kolumn vid läsning.
...
- **Seed-effekten** har en `if (isGuestHost)`-gren FÖRE profil-läsningen. Sedan v2 (2026-07-04) läser den `getLobbySettings(roomCode)` async för de guest-VARIABLA fälten (`gameMode`, `singlePlayerDefault`, `roundsCount` clampad till {2,4}, `spotifyEnabled`, samt sedan 2026-08-08 `answerResponseSeconds` clampad till {30,45,60} med **30 som fresh-default**) så "Play Again + keep players" bevarar guest-hostens val (goToNewLobby skriver carry-over-settings till nya koden). Host-kortets `assistance` seedas från `guestAssistance`-paramet (fallback `'full'`). **Game era är guest-VARIABEL sedan 2026-09-02** (Peter): fresh guest-lobby seedar `[guestBirthYear, ERA_MAX]` (clampad som i registrerade grenen — `to` golvas till `ERA_TO_MIN`, `from` till `[ERA_MIN, to − ERA_MIN_INTERVAL]`), carry-over läser `stored.eraFrom/To`, och slidern renderas/editeras precis som för en registrerad host. Låsta fält hårdkodas alltid: maxPlayers 4, Sweden, inga paket, alla source-kategorier ON. `lobbySeededRef` sätts inne i `.then` (debounce-race). Debounce-skrivningen persisterar till `lobby_settings` som vanligt så non-hosts syncar.
- **Dolt/låst UI** (allt på `isGuestHost`, delad not-stil `styles.guestHostNote`): credits-pill + Share invite dolda; Number of Rounds = två val-rutor **2 / 4** i `modeOption`-stil (stepper + RoundsRuler + game-mode quick-select-raden renderas inte) + not "Upto 20 rounds option for registered Quizvibe users with Premium"; Players visar bara Max 4-rutan (+ spacer, Max 12 dold) + not "Upto 12 players option for registered Quizvibe users with Premium"; Host packages visar Generic + "Activate Extra package"-knappen med grå PREMIUM-badge (2026-07-07 — extraPackagesWrapper dold, not "Extra packages only available for registered QuizVibe users with Premium"; tap på Activate → register-popup vars Yes raderar lobbyn via `performLobbyDelete` + öppnar Register-formuläret på Home via `?openRegister=1`).
- **Fria settings**: **Answer response time** (30/45/60 sedan 2026-08-08 — raden renderas som för registrerade hosts; noten "Option to select 30s or 45s not available for Guest user" borttagen) och **Game era** (sedan 2026-09-02 — slidern renderas som för registrerade hosts, default `[guestBirthYear, idag]`).
- **Assistance LÅST till Full (2026-09-12)**: guest host:s EGET kort går inte att ändra i player-edit-sheeten — `editAssistanceLocked = (isGuestHost && playerEditTarget?.isHost) || playerEditTarget?.addedByHost` visar en statisk Full-chip + noten "Guests play on Full assistance." (samma statiska-chip-mönster som remote mutual assistance). Samma lås gäller **host-tillagda gäster** (`+ Add Player`): AddPlayerModal har ingen assistance-väljare längre (statisk Full-chip), och `handleAddPlayer` forcerar `assistance: 'full'`. **Självanslutna gäster som JOINAR** en multiplayer-lobby är INTE låsta (de valde nivå på guest-JOIN-formen, ingen `addedByHost`-flagga) — host kan fortsatt editera dem. `handleStartGame`/`goToNewLobby` bär genomgående `'full'` för guest host:s rad.
...
2. **Övriga frågor (Peter 2026-09-09):** (a) **ANTALET YouTube- vs Hints-frågor är GARANTERAT jämnt delat per spel** — 4 rundor utan Spotify → exakt 2 YT + 2 Hints (jämnt `weightedTotal` → hälften var; udda → myntkast om extra-slotten). (b) **Music vs Film är en VIKTAD 75/25-sannolikhet INOM varje källa** (förväntad proportion över många spel; får skilja sig mellan enskilda spel). Sport utgår i v1. Två per-källa-celler (Music-vikt 0.75 / Film-vikt 0.25) dras via `drawFromCells`-helpern: dedupe mot picked-Set, viktrenormalisering över icke-tomma celler, `pickTiered`-färskhet, **within-source-fallback** (källans egen pool om cellerna töms — så YT/Hints-delningen inte läcker mellan källorna). (c) **Fast presentationsordning: Spotify → YT-Music → YT-Film → Hints-Music → Hints-Film** (Music alltid före Film inom varje källa via `orderByCat`; stray-items sist). IndDev-korrekthet krävs bara på host-enheten (non-host renderar broadcast question_ids via ALL_QUESTIONS_MAP).

## Home — pulserande tagline

Tre taglines som cross-fadar under brand-loggan på Home-skärmen ([app/index.tsx](app/index.tsx)): `"Challenge yourself. Play together."` → `"Invite Friends. Socialize."` → `"Music. Film. Play."`. Module-level `TAGLINES`-array driver array av per-text Animated.Value (initierad så index 0 startar på opacity 1). Cycle var 6000ms: parallell `Animated.parallel`-fade där nuvarande text fadar till 0 och nästa till 1, duration 2600ms med `Easing.bezier(0.4, 0, 0.2, 1)` (Material Design standard ease — mjuk accel/decel). useNativeDriver: true så opacity körs på native-tråden. Render: alla TAGLINES alltid renderade i samma wrap (`alignSelf: 'stretch'` + `position: 'relative'`), index 0 i flow definierar wrap-höjden, index 1+ är absolut-positionerade ovanpå via `taglineOverlay` så de delar exakt samma position. Cycle-loop använder `taglineIdxRef` för att spåra current index över React-renders utan att triggera re-renders.
