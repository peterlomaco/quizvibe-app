...
**Svarstyp-badge på current-box** (2026-06-07): kant-skärande badge på **vänster sida** av `currentMediaBox` (IndDev/Single) och `currentPlayerBox` (PtP) — visar `"Year"` eller `"Name"` för frågan som spelas härnäst. Blå styling (`Colors.primary` bg + vit text). Position: `top: -9, left: Spacing.md`. Deriveras som `currentAnswerType: 'Year' | 'Name' | null` via `answerTypeByQuestion`-prop (parallel array med `categoryByQuestion`): Spotify-frågor → `'Year'`, YouTube + Film-kategori → `'Name'` (filmtitel), YouTube övrigt → `'Year'`, image-frågor → `'Name'`. null → ingen badge. `currentMediaNumber` på IndDev-boxen har `left`-positionen bumpad till `Spacing.xxl` (32px) för att ge plats åt badgen.
