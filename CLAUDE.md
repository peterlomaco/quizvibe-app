...
- **Toggeln** bor i GetReadyIntro:s utfällbara **Game settings**-block som en "Audio: On/Off 🔊"-rad (`showSelfAudioToggle = mode === 'remote-1v1' && !!onSelfAudioChange`). Tap växlar direkt — ingen modal, eftersom det bara finns en spelare per enhet att styra (IndDev:s `showAudioTrigger`-rad med per-spelare-modal är orörd). Props: `selfAudioOn` + `onSelfAudioChange`; quiz.tsx skickar callbacken bara när `gameMode === 'remote-1v1'`, så lokala lägen renderar ingen rad. **Raden visas för BÅDA spelarna** — motståndaren är sin egen "host" i sin solo-session och måste kunna tysta sin egen enhet.
...
| **Claim** | EN användares gratismånad — startar vid tryck på Free, slut en kalendermånad senare | Användaren, per enhet |
...
**Gäller/gäller inte:**
- **Single player** (`turnOrder.length === 1`): ingen invite-fråga (ingen att bjuda in) — panelen fälls ut direkt med `rematchInvite=true`, Remote Play står kvar, och Local Play går till `askKeepSettingsThenGo(true)` (= exakt gamla single-player-Play-Again-beteendet med Cancel-utväg).
...
- **Guest-hostade spel**: SAMMA flöde, med tre skillnader. (a) **Remote Play visas aldrig** — remote 1vs1 är QuizVibe-users-only. (b) **Keep/Reset-prompten hoppas över** — guest-lobbyn har få valbara settings, så Local Play går direkt till `goToNewLobby(true, true)` (= behåll dem). (c) **1-replay-cappen ligger kvar**: `localRematchFlow` kräver `guestReplaysUsed < 1`, så omgång 2 renderar ingen "Start New Game" alls — bara Home, exakt som förr. Noten "Replay only possible 1 time for Guest Hosts" är flyttad till **överst i sticky-footern** (ovanför knappen) eftersom guest hosts numera tar Home-only-grenen där den gamla placeringen låg.
- **Remote 1v1**: oförändrat (egen `startNewGameLocked`-logik, se remote-sektionen).

**Implementation:**
- `RoundLeaderboard` fick fem props: `onStartNewGamePress` (tappet lämnas till call-siten i stället för att toggla), `startNewGameExpanded` (**kontrollerat** öppet-läge; utelämnas → internt state, remote-fallet), `localPlayLocked` + `onLocalPlayLockedPress`, `hideRemotePlay` och `startNewGameNote` (ReactNode under panelen). Host:s Play Again-knapp renderas inte när `isHost && onStartNewGame` (`hostUsesStartNewGame` → Home-only-grenen, samma som remote redan använde via guestHost-hacket).
- `HostTypeOptions` fick `localLocked` + `onLocalLockedPress` (speglar den låsta Remote-radens mönster: raden dimmas men förblir tappbar så trycket kan förklara väntan).
