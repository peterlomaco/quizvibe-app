...
**Image-validering 2026-05-31** (Peter-genomgång): 94 oönskade bild-items togs bort HELT — webp + katalog-poster + require-map + fråge-data (1100 → 1006). Verktyg som driver valideringen (alla i `backend/scripts/`, kör via `tsx`):
- **`review-quiz-images.ts`** (`npm run review-quiz-images`) — HTML contact-sheet över alla webps grupperade per subject, thumbnail + namn + id + upplösning, flaggar low-res (<0.5 MP) + orphans. Primär visuell granskningsmetod. Output: `backend/output/quiz-images-review.html`.
- **`review-non-swedish-images.ts`** (`npm run review-non-swedish-images`) — HTML contact-sheet fokuserad på icke-Sverige-specifika bildfiler (filer utan "sweden" i filnamnet). TOC med ankarlinks per fil, streckad ram = saknar webp. Används för regional curering (ta bort / parkera icke-svenska items). Output: `backend/output/non-swedish-review.html`.
...
**Curation-scripts** (alla i `backend/scripts/`):
   ...
   - `scripts/sync-quiz-images.ts` — regenererar `src/utils/quizImages.ts` med require()-statements för alla webp i assets/quiz-images/.
...
**UPPDATERAD 2026-08-07 (rev 3): layouten är TVÅ rader (Single ensam på rad 1; PtP + IndDev på rad 2). Remote (1vs1) visas INTE som Game Mode-ruta — 1vs1 väljs via `HostTypeOptions`-utfällningen på Home ("Start New Game" → "Local Play" eller "Remote Play") och får en egen renodlad lobby-vy. Remote Play kräver ett QuizVibe-konto — se sektionen "Remote 1v1" nedan.** Beskrivningen nedan (2026-06-01) gäller i övrigt oförändrat för Single/PtP/IndDev.
...
**Gemensam Assistance level (Peter 2026-08-08, migration `0033_lobby_settings_remote_assistance.sql`)** — i lokala lägen är assistance en PERSONLIG inställning per spelarkort, men i remote svarar båda på samma frågesekvens var för sig, så olika nivåer gör duellen ojämförbar. Host väljer därför **EN nivå för båda** i 1vs1-lobbyn.
- **UI**: rad direkt under "Remote play — 1vs1"-indikatorn i Game Mode-sektionen — `REMOTE_ASSISTANCE_OPTIONS` renderade som `modeOption`-rutor (**Full / Medium / Minimal**), info-ikon + not "Applies to both players" (host) / "Selected by the Host" (non-host). Följer render-för-alla-men-`disabled={!hostMode}`-mönstret. ⚠ **Mellannivån heter "Medium" HÄR men "Standard" i övriga appen** (Profile, Add Player, player-edit, leaderboard-metaraden) — Peters formulering; underliggande värdet är samma `'standard'`. Ett app-brett rename är ett separat pass över `ASSISTANCE_LABEL` + `ADD_PLAYER_ASSISTANCE_OPTIONS` m.fl.
- **Default `'full'`** — MEDVETET inte hostens profil-assistance: nivån gäller båda spelarna och ska inte tyst påtvingas motståndaren. Carry-over (Play Again) vinner över defaulten.
- **State + sync**: `LobbySettings.remoteAssistance` (`LobbyRemoteAssistance = 'full'|'standard'|'minimal'`, typguard `isRemoteAssistance`). ⚠ Kolumnen ingår MEDVETET INTE i `settingsToRow` — `setLobbySettings` gör en **separat targeted UPDATE** som dessutom är gated på `gameMode === 'remote-1v1'`. Samma skäl som `sketch_enabled`/`spotify_answer_*`: en upsert som nämner en okörd kolumn failar HELA settings-skrivningen och bryter all lobby-sync, även i lokala lägen. Med separat UPDATE degraderar en icke-applicerad 0033 till en console.warn och `rowToSettings` defaultar `'full'`.
- **Under spel är MATCH-snapshotten sanningen**: `handleStartGame` skriver `remoteAssistance` till **BÅDA** `remote_match_players.assistance`-raderna. Alla tre vägar in i quiz läser därifrån — `buildRemoteQuizParams` (`me.assistance`, 1vs1 Matches + kod-återinträde), lobbyns non-host game-started-gren (`matchAssistance` = egen rad → motpartens → `selfRow.assistance` som legacy-fallback) och hostens egen nav. ⚠ Hostens nav måste sätta nivån **i `players`-arrayn**, inte bara i `params.assistance` — quiz.tsx läser `turnOrder[currentPlayerIndex]?.assistance` primärt och paramet bara som fallback.
- **Per-spelare-vägar är låsta i remote**: player-edit-sheetens Assistance-rad visar den gemensamma nivån statiskt + pekar på Game Mode-kontrollen (samma mönster som guest-hostens Full-lås), och `PlayerRow`-pillret visar `remoteAssistance` i stället för `player.assistance` så korten inte motsäger sektionen ovan.
...
- **Sekvens-auktoritet**: init-effekt (efter gameQuestionsRef) hämtar matchen; `question_ids` null + host → persistera lokala sekvensen EN gång; annars poll tills satt. `gameQuestions`-useMemo:n har remote-override överst: `remoteQuestionIds.map(id => ALL_QUESTIONS_MAP.get(id))` — host-resume och motståndare renderar identiskt. Render-gate ("Preparing 1vs1 match") blockerar allt spel-UI tills sekvens + resume-seed är klara. `totalQuestions` clampas mot sekvens-längden (katalog-skew → ids filtreras, saknas ALLA → "Update required"-Alert).
  ...
  - ⚠ **Allt slumpat frågeinnehåll MÅSTE vara deterministiskt i remote** (fix 2026-08-08). De två spelarna bygger sina ledtrådar och svarsalternativ LOKALT ur samma katalog utan sync-kanal — med `Math.random` fick de olika distraktorer, olika hints och olika ordning på exakt samma fråga (orättvis duell). Tre saker gör utfallet identiskt:
    ...
    3. **Gemensam Assistance** (se nedan) — sista variabeln som kunde ge olika svarsblock. Med (1)+(2)+(3) är underlaget nu bit-identiskt för båda spelarna.
...
- **Avslut**: leaderboard-effekten anropar `finalizePlayer` (sista finishern triggar vinnarberäkningen server-side). `appendGameHistoryEntry` SKIPPAS för remote (server-tabellerna är historikkällan — annars dubbelräkning i Player History). Final-footern återanvänder guest-flödets "bara Home" (`guestHost=true + guestReplaysUsed=1` → Play Again dold) **plus en gold "Start New Game"-knapp** (2026-08-08) ovanför Home-raden: samma vokabulär som Home:s knapp (56 px, `Colors.warning` bg+kant, svart text) med SAMMA inline-`HostTypeOptions`-utfällning (Local Play / Remote Play). Drivs av `RoundLeaderboard.onStartNewGame?: (lobbyType) => void` — utelämnas → knappen renderas inte alls, så lokala lägen är orörda; quiz.tsx skickar den bara när `isRemote && isLastQuestion && !isGuestHostGame`. Medan panelen är utfälld göms Home/Play Again-raden (speglar Home som döljer sina övriga knappar). Handlern `handleStartNewGameFromFinal` i [app/quiz.tsx](app/quiz.tsx) speglar Home:s `handleCreateGame` steg för steg (credit-gate → `registerActiveRoom` med `isRemote1v1` → per-rum-cleanup → `router.replace('/lobby')`) men rör ALDRIG den pågående matchens rum — motståndaren har 48h kvar (samma resonemang som `handleGoHome`:s remote-gren).
...
- **`handleSaveRemoteLobby(fromHostSheet)`** i [LobbyScreen.tsx](src/screens/LobbyScreen.tsx): stänger sheet:en → `saveLobby(...)` → Alert ("Find it under Remote Play History … status Not started") → `router.replace('/')`. Medvetet INTE Leave/Delete: non-host markeras **aldrig** `hasLeft` (behåller sin plats så host fortfarande kan trycka Start Game), host raderar **inte** rummet (koden är fortsatt joinbar).
...
- `creditsPill`: 1 px `Colors.primaryBorder`, `Colors.cardElevated` bg, `Radius.md`, `minWidth: 210`.
...
- **Premium**: kant-skärande **UNLIMITED-badge** (`creditsMembershipBadge`) + `creditsPillMembership`-styling.
...
**Expo Go-skydd (2026-05-29):** `configurePurchases` i [src/lib/iap.ts](src/lib/iap.ts) wrappar iOS-konfig-anropet i try/catch. RevenueCats native-modul saknas i Expo Go → utan skyddet kraschar appen vid start (vit skärm). Med skyddet fångas felet, `configured` förblir false, och alla övriga IAP-anrop no-op:ar via sina `!configured`-guards. IAP funkar normalt i dev-/standalone-builds (där native-modulen finns). Detta gör Expo Go användbart igen för snabb JS-iteration. **OBS:** TestFlight-byggen har JS:en INBÄDDAD från byggtid → laddar INTE live kod från Metro; använd Expo Go (laddar live) eller ett riktigt development build för att se nya ändringar.
...
- Lobby: `spotifyConnected`-state/fält betyder nu "self-attestad", seedas från `profile.spotifyAppConfirmed` (host focus-effect + non-host code-only-join). Lobby-radens "I have the Spotify app"-länk (f.d. Connect) attesterar in-lobby: `handleConnectSpotify`/`handleDisconnectSpotify` är lokala attest-handlers som behåller sina side-effects (spelarkort-patch + `upsertOwnLobbyPlayer` → `spotify_verified`) och skriver tillbaka `spotifyAppConfirmed` till profilen.
...
- `LobbyPlayer.spotifyConnected?: boolean` — populerad från `lobby_players.spotify_verified` via `rowToPlayer`; betyder self-attest (Plan B). Host-kortets `spotifyConnected` sätts från Lobby `spotifyConnected`-state i `useFocusEffect` (seedas från `profile.spotifyAppConfirmed`). Non-host vid code-only-join: `profile.spotifyAppConfirmed` i samma `loadProfile()`-läsning.
...
**Formulär** (JoinModal-steg `'guest-host'` i [app/index.tsx](app/index.tsx)): delar form-stomme med guest-JOIN-steget (samma state/handlers — PlayerName-split-field med auto-fill `GuestX-1234567` + Check/Auto-generate/Remove, samma Year of birth-picker). Skiljer sig: titel "Start Game as Guest", INGEN assistance-väljare, INGA rumkods-celler; istället två read-only-rader "Answer response time: 60s" + "Assistance level: Full" (`modal.guestFixedRow/-Label/-Value`). Submit "Start Game as Guest" (`handleStartGameAsGuestHost`): kör `ensureAuthSession()` FÖRST (anon-signup; abort med Alert vid null — annars no-op:ar `registerActiveRoom` tyst och joiners får "Room not found") → `generateRoomCode` → `registerActiveRoom` (maxPlayers 4, hostIsPremium false, hostPlayerName = guest-namnet så own-lobby-detektering fungerar) → cleanup-bunten → `router.push('/lobby', { code, isHost:'true', guestHost:'true', guestName, guestBirthYear })`.
...
- **Seed-effekten** har en `if (isGuestHost)`-gren FÖRE profil-läsningen. Sedan v2 (2026-07-04) läser den `getLobbySettings(roomCode)` async för de guest-VARIABLA fälten (`gameMode`, `singlePlayerDefault`, `roundsCount` clampad till {2,4}, `spotifyEnabled`) så "Play Again + keep players" bevarar guest-hostens val (goToNewLobby skriver carry-over-settings till nya koden). Låsta fält hårdkodas alltid: maxPlayers 4, Sweden, **60s**, era `[ERA_MIN, ERA_MAX]` (fulla spannet), inga paket, alla source-kategorier ON. `lobbySeededRef` sätts inne i `.then` (debounce-race). Debounce-skrivningen persisterar till `lobby_settings` som vanligt så non-hosts syncar.
- **Dolt/låst UI** (allt på `isGuestHost`, delad not-stil `styles.guestHostNote`): credits-pill + Share invite dolda; era-slidern dold (display-boxen visar spannet + not "change Game era not available for Guest user"); response time visar ENDAST 60s-knappen + not "Option to select 30s or 45s not available for Guest user"; Number of Rounds = två val-rutor **2 / 4** i `modeOption`-stil (stepper + RoundsRuler + game-mode quick-select-raden renderas inte) + not "Upto 20 rounds option for registered Quizvibe users with Premium"; Players visar bara Max 4-rutan (+ spacer, Max 12 dold) + not "Upto 12 players option for registered Quizvibe users with Premium"; Host packages visar Generic + "Activate Extra package"-knappen med grå PREMIUM-badge (2026-07-07 — extraPackagesWrapper dold, not "Extra packages only available for registered QuizVibe users with Premium"; tap på Activate → register-popup vars Yes raderar lobbyn via `performLobbyDelete` + öppnar Register-formuläret på Home via `?openRegister=1`); player-edit-sheetens assistance-rad låst till statisk "Full"-chip när guest host editerar sitt EGET kort (year fortsatt redigerbart; andra spelares kort fritt).
