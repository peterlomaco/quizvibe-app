...
### Marathon-listan (Profile) — kort-layout, sortering, filter, re-match (2026-09-10)

[SavedAggregatesCard](src/components/SavedAggregatesCard.tsx) på Profile omstrukturerad. Grupperingen delas fortsatt med per-spel-historiken via [historyGrouping.ts](src/utils/historyGrouping.ts) (`groupHistory`, ren + vitest-testad).
...
- **Sist spelad-datum** i radens övre högra hörn (`item.updatedAt ?? item.createdAt`, `toLocaleDateString('en', …)`), ovanför chevron:en.
- **Tre sorteringar** (`SortMode = 'host' | 'date' | 'name'`): **Host Name** (DIN egen host överst, sedan alfabetiskt, "Unknown host" sist), **Date** (last-played, `updated_at`, oförändrat), **Table Name** (alfabetiskt på tabellnamnet, ny). Self-first drivs av `getCurrentUserId()` ([auth.ts](src/utils/auth.ts), ny helper) → jämförs mot `item.createdBy` via `groupHistory`:s nya `getHostUserId`-accessor + `selfUserId`-param (host-läge sätter `isSelf` på hinken). `getName`-accessor + `'name'`-läge lades till samtidigt. Alla tillägg är optional → per-spel-historiken (`PlayerHistorySection`) är oförändrad.
...
- **Filtrera på QuizVibe-friend** via en bottom-sheet-picker (mönster som Lobbyns Share invite): `loadFriends()` → lista med **kryssruta per friend + "Add"-knapp** på "All players"-raden. Multi-select → **AND-filter**: bara marathons där ALLA ikryssade deltog. Friends bär inget `userId`, så matchning sker på `playerName` (case-insensitivt) mot deltagarnas snapshot-namn. Tom träff → "No marathons where all selected players took part." Picken öppnas seedad ur det applicerade urvalet; Close = avbryt.
