...
**Region-tag** (2026-05-26): alla 19 catalog-filer har `region: ["sweden"]` på fil-headern (V1-default). `RegionSchema` = `'sweden' | 'nordics' | 'europe' | 'global'`. Items kan bära `region`-override för bredare recognition — t.ex. `["sweden", "global"]` för internationellt iconic-figurer. **Curator-regel**: alla nya items DEFAULT till fil-headerns `region: ["sweden"]`. Endast item-override om curatorn explicit vill markera item som global-iconic. V1-spelaren har hardcodad region `sweden`, så multi-region har ingen runtime-effekt än — men data är forward-compat förberedd för V2-expansion. Filter-semantik (V2): visa item om `item.region` intersects `player.region` ELLER innehåller `'global'`. Se `memory/project_v1_v2_region_strategy.md` för full spec.
