...
- **Tillämpas** i [quiz.tsx](app/quiz.tsx) `gameQuestions` via `applyItemHcp(pool)` på de tre pool-erna (spotify/pureYoutube/image), PER KATEGORI via `filterPoolByCategoryHcp` (Music-fråga mot Music-HCP osv.). `HCP_FILTER_MIN_POOL = 30`. Gäller BARA **Single Player + Pass-the-Phone**; **IndDev** (delad host-sekvens), **remote** (server-sekvens) och **guest-hostade** spel filtreras INTE. Läser DENNA enhets per-kategori-HCP via `loadOwnCategoryHcp(region)`.
...
- **Fria settings**: **Answer response time** (30/45/60 sedan 2026-08-08 — raden renderas som för registrerade hosts; noten "Option to select 30s or 45s not available for Guest user" borttagen), **Assistance level** (Full/Standard/Minimal sedan 2026-08-08 — player-edit-sheeten har inte längre ett statiskt "Full"-chip för guest host:s EGET kort; alla kort editeras lika) och **Game era** (sedan 2026-09-02 — slidern renderas som för registrerade hosts, default `[guestBirthYear, idag]`). Nivån följer med hela vägen: Home-formen → `guestAssistance`-param → host-kortets seed → `handleStartGame`:s quiz-param (`turnOrder.find(id === '1')?.assistance`) → `goToNewLobby`:s carry-over (som tidigare forcerade `'full'` på guest-hostens rad).
