...
**Pool-status idag (1006 live image-webps efter validerings-purge 2026-05-31)** — V1-curering utifrån svensk igenkänning (`region: sweden` = recognition i Sverige, NOT nationality — Adele, Drake etc. är taggade `region: sweden` eftersom svenska spelare känner igen dem). Action-shot-policy-audit per bucket (`feedback_image_professional_context.md`) är paussad till mass-curation klar; porträtt-fallback godtagbar för items utan tillgänglig action-shot per 2026-05-27.
...
2. **Image pool expansion svenska artists/bands** (svenska audit-pass 2026-05-27 del 2): 14 nya webp processade — 10 action-shots (live från scen): Cleo (Way Out West 2014), Sarah Klang (Malmöfestivalen 2018), Newkid (Kungsan), Adna (Piknik i Parken 2017), Sanna Nielsen (Melodifestivalen 2025), Familjen (Andreas Tilliander live), Nationalteatern (2013 live), Noice (Bingolotto 2023), Larz-Kristerz (Viking Cinderella), Lordi (3-Majówka 2023). 3 porträtt-fallback: Patrik Isaksson, Pernilla Andersson, Docenterna. 1 audit-fix: Erik Lundin (rappare — tidigare felmatch mot Anders Lundin via specifik sökhint; plain-name `Erik Lundin rappare` triggar sv:Erik Lundin (rappare)-pageimage). **7 svenska items kvar utan webp** (Hurula, John Dahlbäck, Freestyle (svensk popgrupp), Ratata, Lolita Pop, Tjuvjakt, Eva Remaeus) — varken Wikipedia pageimage eller Commons text-search ger användbara träffar; alla queries returnerar felmatch (logos, PDFs, unrelated photos). För dessa krävs Commons Category-page-research direkt via browser: `https://commons.wikimedia.org/wiki/Category:<name>` — om kategorin existerar, picka manuellt; annars Wikipedia-artikel saknas helt och inga PD/CC-bilder finns publicerade.
3. **Image audit fortsättning** (PRIO 2 enligt Peter) — svenskt-fokus där relevant. Återstående buckets:
...
4. **Workflow per item** (etablerat 2026-05-27): `npm run wikimedia-search <id>` → om bara post-career, curl Commons-kategori → välj action-shot från lista → `npm run wikimedia-process <id> <url>` → `cp backend/output/<id>.webp assets/quiz-images/<id>.webp`. **För batch-pass** (default-hints triggar inte): plain-name-probe via [backend/scripts/quick-pageimage-probe.ts](backend/scripts/quick-pageimage-probe.ts) → samla URLer i JSON → [batch-wikimedia-by-url.ts](backend/scripts/batch-wikimedia-by-url.ts) `--json <path>` processar alla på en gång. Spara JSON i `backend/scripts/batch-input-*.json` så framtida sessions kan referera ihop med commit-historiken. Useful sources: Lipofsky (NBA), Anefo "in aktie" (70-80-tal sport), AFP/Scanpix PD (60-tal), Freiburg LABW archive.
...
2. **Curation-scripts** (skapade 2026-05-26):
   ...
   - `scripts/batch-wikimedia-process.ts` — batch Wikipedia pageimage → sharp → assets/quiz-images/. Engelska först, svenska fallback. Bra för items med korrekt `wikimediaSearchHints[0]`.
   - `scripts/batch-wikimedia-by-url.ts` (2026-05-27) — batch-process via explicit URL-per-item (`--json <path>`). Använd när default-hints triggar fel pageimage (specifika 5-ords-hints matchar inte mot artikel-titlar) eller när URLer är hand-pickade från probe-script.
   - `scripts/quick-pageimage-probe.ts` (2026-05-27) — testar plain-name-queries mot Wikipedia pageimage (sv+en) + Commons text-search fallback för items utan triggning. Edit hardcoded array per session.
   - `scripts/find-missing-webps.ts` (2026-05-27) — listar alla image-items i katalog som saknar `assets/quiz-images/<id>.webp`, grupperade per YAML-fil. Run efter mass-expansion för att hitta gaps.
...
- **`enabledHostPackages?: string[]`** på `ProfileData` — lista över paket-id:n som är aktiverade.
...
- **Profile-UI** ([ProfileScreen.tsx](src/screens/ProfileScreen.tsx)): `availablePackages = [...PURCHASED_PACKAGES]` via `useMemo` — i V1 alltid tom. Empty-state-texten "No host packages available" renderas. Save-knappen + sub-rubriken finns kvar för struktur-konsistens; aktiveras när themed packages introduceras i v1.1+.
...
**Profiles-källan** (2026-05-30, f.d. "Images"-raden): en förälder-rubrik (`connectionSubHeading`) med TVÅ under-toggles som linjerar exakt med YouTube-raden (plain `connectionRow`): **Images** (foto-biblioteket, 1+2 — Q-"?"-ikon, `imagesEnabled`) och **Sketch** (AI-doodlen, "Guess Who" — penn-ikon, `sketchEnabled`). Host kan ha BÅDA, ENA eller INGEN aktiverad. **Images räknas i min-1-guarden** (`handleToggleSource`/`enabledSourceCount` = youtube+images); **Sketch är en FRI toggle** (default AV, `onValueChange={setSketchEnabled}` utan guard) eftersom doodlen INTE är wirad till quiz-poolen ännu (prototyp) → en sketch-only-lobby skulle ge tom pool. När doodlen kopplas in: lägg `sketchEnabled` i `enabledSourceCount` + skicka som URL-param till quiz. **DB:** `sketch_enabled`-kolumnen (migration `0013_sketch_enabled.sql`, manuell apply) skrivs INTE av `settingsToRow` än (en upsert mot okänd kolumn failar HELA lobby-synken) — tolerant read `?? false`. Efter migration: avkommentera write-raden i [mockLobbySettings.ts](src/utils/mockLobbySettings.ts) för cross-device sketch-sync. Tills dess host-lokal.
