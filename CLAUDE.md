...
**UPPDATERAD 2026-08-12 (rev 4): layouten är ÅTER EN RAD med tre rutor — Single player / Pass-the-Phone / Individual device — med bracket-etiketterna "SINGLE MODE" (flex:1, under ruta 1) och "MULTIPLAYER" (flex:2, spänner över ruta 2-3) under raden.** Rev 3 (2026-08-07) splittade den i två rader när Remote (1vs1) låg som fjärde ruta; när Remote flyttades till Home-valet blev tvåradsvarianten kvar av misstag och är nu återställd. Samma en-rads-layout gäller på ALLA fyra call-sites: Lobbyns Game Mode-sektion + dess quick-select under RoundsRuler, samt motsvarande två i Profile:s host-defaults. **Remote (1vs1) visas INTE som Game Mode-ruta** — 1vs1 väljs via `HostTypeOptions`-utfällningen på Home ("Start New Game" → "Single Game" / "Multiplayer Game" / "Remote Play") och får en egen renodlad lobby-vy. **Single vs Multiplayer väljs sedan 2026-08-24 också DÄR** — se "Lobbytyp-param styr singlePlayerDefault-seeden" direkt nedan. Remote Play kräver ett QuizVibe-konto — se sektionen "Remote 1v1" nedan. Beskrivningen nedan (2026-06-01) gäller i övrigt oförändrat för Single/PtP/IndDev.
...
Struktur (per skiss):
- EN `modeRow` med tre likbreda rutor: **Single player** · **Pass-the-Phone** · **Individual device**.
- Bracket-rad direkt under (`multiplayerBracket` + `multiplayerBracketLabel` + info-ikon): **"SINGLE MODE"** i en `flex:1`-kolumn under ruta 1, **"MULTIPLAYER"** i en `flex:2`-kolumn som spänner över ruta 2-3. Flex-talen MÅSTE spegla antalet rutor de sitter under — det är just den kopplingen som gick sönder när Remote lades till/togs bort.
...
**Lobbytyp-param styr singlePlayerDefault-seeden (2026-08-24).** `HostTypeOptions`-utfällningen (Home + Final Leaderboard) skickar `lobbyType: 'single' | 'multiplayer' | '1v1'` som URL-param till `/lobby`. LobbyScreen:s `resolveSeedSinglePlayer(lobbyType, fallback)` ger prio **param > carry-over `lobby_settings` > profilens host-default > false**, och används i BÅDA seed-grenarna (guest-host + registrerad host). Den är MEDVETET module-level och ren, så den aldrig blir en dependency i seed-effekten — `lobbyType` ligger redan i dess dep-array och är allt funktionen läser.
...
- **`gameMode` rörs INTE av paramet** — vilket MULTIPLAYER-läge som förvaljs kommer fortsatt ur kedjan `stored > profile.spotifyDefaultEnabled → IndDev > profile.gameMode > 'pass-the-phone'` (Peters beslut 2026-08-24: Multiplayer-raden ska garantera multiplayer, inte överrida hostens sparade läge). En host med IndDev som host-default landar alltså i IndDev.
...
- **Game Mode-rutorna renderas som vanligt i båda fallen** — paramet är ett förval, ingen låsning. Host byter fritt i lobbyn.
...
- **V1-scope (2026-05-27):** **Inga paket alls.** Themed packages (Hip Hop / Rock / Film & Actors) är parkerade till v1.1+ och `PURCHASED_PACKAGES`-arrayn är tom. **Generations-paketen ("Play as Gen X" etc.) togs bort 2026-05-27** — Peter beslutade att gen-konceptet inte är aktuellt. `enabledHostPackages` är därmed tom array genom hela V1.
...
- **Profile-UI** ([ProfileScreen.tsx](src/screens/ProfileScreen.tsx)): "Activate Extra package"-knapp (gold/grå badge per `hasPremium`); listan + Select all + Save renderas bara vid Premium. `availablePackages = [...PURCHASED_PACKAGES]` via `useMemo` — i V1 alltid tom → empty-state "No host packages available" (för Premium-user).
- **Lobby-render** ([LobbyScreen.tsx](src/screens/LobbyScreen.tsx)): `allPackagesCatalog = [...PURCHASED_PACKAGES]` (= tom i V1). `availablePackages` skiljer host vs non-host:
  - **Host**: `allPackagesCatalog.filter(p => enabledHostPackages.includes(p.id))` — tom × tom = tom.
...
- **Seeding**: Lobby host-seed-effekten läser `profile.enabledHostPackages` (klampas mot katalogen) och auto-aktiverar `selectedExtraPackages = enabledIds` för Premium-host i fresh lobby; carry-over vinner. Non-hosts ser endast paket som hosten aktiverat för denna lobby via `selectedExtraPackages`.
...
- **Premium auto-aktivering**: host-seed-effekten sätter `selectedExtraPackages = enabledIds` (profil-enabled ∩ katalog) för fresh lobby när `premium`; carry-over (Play Again + Keep settings) VINNER — inkl. medvetet tomt Generic-val — men klampas mot premium (utgången → `[]`) och enabledIds. Mid-session-övergångar via `prevPremiumRef` (null-init) i focus-effekten: lapse → töm selection; köp (false→true) → auto-aktivera bara om selection tom. Gated på `lobbySeededRef`.
...
**Single player → mid-step-popup (2026-08-24).** Tappar host "+ Add Player" medan `singlePlayerDefault` är true visas Alert **"Change to Multiplayer mode?"** ("Adding a player requires a multiplayer mode. Which one do you want to play?") med tre knappar:
- **Pass-the-Phone** → `handleSelectMode('pass-the-phone')` + öppnar Add Player-formuläret.
- **Individual device** → `handleSelectMode('individual-devices')` + en informations-Alert. Formuläret öppnas MEDVETET inte — IndDev blockerar host-tillagda guests (de saknar egen enhet), så spelarna ska joina via rumskoden i stället.
- **Cancel** → allt orört.

Utan mellansteget öppnades formuläret tyst i single-player-läge: lobbyn fick två spelarkort men spelet startade ändå som single player (Peter 2026-08-24). Grenen ligger FÖRST i `handleOpenAddPlayer`, före både "Own device required"-guarden och capacity-checken (en single-player-lobby har bara host, så kapacitet kan aldrig vara problemet där).

⚠ **`ALERT_TO_MODAL_DELAY_MS = 350`** (module-level i LobbyScreen) fördröjer `setAddModalVisible(true)` — att presentera en RN `<Modal>` direkt i en Alert-callback kan sväljas tyst på iOS eftersom alertens dismiss-animation fortfarande pågår. Samma defensiva mönster och samma värde som `MODAL_SWAP_DELAY_MS` i [app/index.tsx](app/index.tsx).
...
**Nuvarande modell**: steg 1 och 2 delar EN render-gren. Primärknappen är alltid den gröna **"Open Spotify"** (`openSpotifyApp`, aldrig omstart). Omstart är en nedtonad sekundär knapp under den — **"Nothing playing? Restart track from the beginning"** (`djRestartTrackBtn`, ingen fyllnad + dämpad kant så ett felaktigt tap är osannolikt). Den är semantiskt korrekt i BÅDA lägena: DJ:n väljer den bara när inget spelas (misslyckad autoplay, typiskt varm Spotify-session efter Play Again där förra låten ligger kvar pausad i mini-playern), och då är omstart precis vad man vill ha oavsett om klockan tickar. `djStep` är oförändrad — den styr fortfarande guide-stegens highlight.
...
## Start New Game på Final Leaderboard (lokalt spel + guest-hostat) — 2026-08-08, omarbetad 2026-08-24

Host:s **Play Again** är borttagen ur slutskärmen och ersatt av samma gula **"Start New Game"** som på Home (`startNewGameBtn` — 56 px, `Colors.warning` bg+kant, svart text). Footer-raden blir Home-only.

**⚠ Ordningen VÄNDES 2026-08-24: LÄGET väljs FÖRST, invite-frågan kommer efter.** Tidigare (rev 1) sköts en "Replay and Aggregate Leaderboard"-Alert in MELLAN tappet och lägesvalet — men när Local Play splittades i "Single Game" + "Multiplayer Game" gick det inte längre: frågan om spelarna ska följa med är bara meningsfull när host väljer SAMMA läge som just spelades. Beskrivningar av en invite-fråga före panelen är stale.

**Flöde (host):** tap → credit-gate → utfällningen (Single Game / Multiplayer Game / Remote Play)
- **rad ≠ `previousLocalMode`** → färsk lobby direkt i det valda läget, **ingen popup**: `goToNewLobby(false, false, undefined, lobbyType)`. Varken spelarna eller lobby-inställningarna hör hemma i det andra läget, så `keepSettings=false` och LobbyScreen seedar från host-profilen (Peters beslut 2026-08-24).
- **rad = `previousLocalMode`** → `Alert "Replay and Aggregate Leaderboard"`
  - **Cancel** → panelen står kvar öppen, inget state ändras.
  - **No, start fresh** → `goToNewLobby(false, true, undefined, lobbyType)`. ⚠ `keepSettings` är **true** här — "fresh" gäller SPELARNA, inte inställningarna; samma läge spelas igen så era/rundor/källor är fortsatt relevanta. Oförändrat sedan 2026-08-08.
  - **Yes, same players again** → `rematchInvite=true`; **Remote Play göms** (lokala spelare kan inte bäras in i en duell). **IndDev**: `play_again_initiated` broadcastas så non-hosts "Approve / Replay"-knapp tänds, och **panelen står kvar öppen med BARA den valda raden grå** (`lockedLocalTypes={[previousLocalMode]}`) + statusraden "Waiting for N of M players to approve" + `SequentialDots` → "✓ All players have approved". Host tappar raden igen när den tänds → `askKeepSettingsThenGo(…, lobbyType)` → `goToNewLobby(true, keep, undefined, lobbyType)` (oförändrat carry-over-maskineri inkl. `play_again_lobby_ready`-broadcasten). **PtP**: inget att vänta in → panelen stängs och Keep/Reset-prompten kommer direkt.
- **Remote Play** → `handleStartNewGameFromFinal('1v1')`, egen lobby från scratch.

Den ANDRA lokala raden förblir valbar under IndDev-väntan — att byta läge startar ändå från scratch och kräver inget godkännande. Låsta radens tap Alert:ar "Waiting for players … Tap this mode again when they have — or pick the other mode to start fresh."

Andra tappet på "Start New Game" stänger panelen igen (toggle) — host:s andra utväg tillbaka till leaderboarden. Öppnas den på nytt nollställs `rematchInvite`, men `playAgainApprovals` behålls, så ett redan godkänt läge låses upp direkt när host svarar "Yes" igen.

**`previousLocalMode`** (`app/quiz.tsx`) = `isLocalSoloGame ? 'single' : 'multiplayer'`, där `isLocalSoloGame` (`gameMode !== 'remote-1v1' && turnOrder.length <= 1`) redan fanns.

**Gäller/gäller inte:**
- **Single player** (`turnOrder.length === 1`): "Single Game" = samma läge → ingen invite-fråga (ingen att bjuda in), `rematchInvite=true` sätts direkt och `askKeepSettingsThenGo(true, 'single')` visas (= exakt gamla single-player-Play-Again-beteendet med Cancel-utväg). "Multiplayer Game" → färsk lobby, ingen popup. Remote Play står kvar.
- **Pass-the-Phone**: invite-frågan visas, men ingen väntan/låsning — alla sitter på samma enhet (`rematchNeedsApproval` kräver `gameMode === 'individual-devices'`).
- **Guest-hostade spel**: SAMMA flöde, med tre skillnader. (a) **Remote Play visas aldrig** — remote 1vs1 är QuizVibe-users-only. (b) **Keep/Reset-prompten hoppas över** — guest-lobbyn har få valbara settings, så `proceedWithRematch` går direkt till `goToNewLobby(true, true, …)` (= behåll dem). (c) **1-replay-cappen ligger kvar**: `localRematchFlow` kräver `guestReplaysUsed < 1`, så omgång 2 renderar ingen "Start New Game" alls — bara Home, exakt som förr. Noten "Replay only possible 1 time for Guest Hosts" är flyttad till **överst i sticky-footern** (ovanför knappen) eftersom guest hosts numera tar Home-only-grenen där den gamla placeringen låg.
- **Remote 1v1**: oförändrat (egen `startNewGameLocked`-logik, se remote-sektionen). Dess panel visar alla tre raderna, och `handleStartNewGameFromFinal` tar numera hela `HostLobbyType` och forwardar paramet.

**Implementation:**
- `RoundLeaderboard` fick fem props: `onStartNewGamePress` (tappet lämnas till call-siten i stället för att toggla), `startNewGameExpanded` (**kontrollerat** öppet-läge; utelämnas → internt state, remote-fallet), `lockedLocalTypes` + `onLocalPlayLockedPress`, `hideRemotePlay` och `startNewGameNote` (ReactNode under panelen). Host:s Play Again-knapp renderas inte när `isHost && onStartNewGame` (`hostUsesStartNewGame` → Home-only-grenen, samma som remote redan använde via guestHost-hacket).
- `HostTypeOptions` fick `lockedLocalTypes: readonly LocalLobbyType[]` (var boolean `localLocked` t.o.m. 2026-08-24) + `onLocalLockedPress` — speglar den låsta Remote-radens mönster: raden dimmas men förblir tappbar så trycket kan förklara väntan.
- [app/quiz.tsx](app/quiz.tsx): `handleLocalStartNewGamePress` (credit-gate + öppna panelen) → `handleLocalStartNewGameSelect` (grenarna ovan) → `proceedWithRematch` (Keep/Reset eller guest-genvägen) + state `startNewGameExpanded`/`rematchInvite`. Credit-gaten är utbruten till **`ensureHostCreditsForNewGame()`** (guest host → alltid true, Premium → true, annars tre-vägs-popupen med Purchase subscription / Restart as Guest / Exit).
- `goToNewLobby` och `askKeepSettingsThenGo` tar båda ett avslutande `lobbyType: LocalLobbyType = 'multiplayer'` som forwardas som `/lobby`-param. De dormanta Play Again-vägarna skickar `previousLocalMode` så deras beteende är oförändrat.
...
> ⚠ Sedan 2026-08-08 nås detta flöde från **"Start New Game" → läges-raden → "Yes, same players again"** (lägesvalet kommer FÖRST sedan 2026-08-24) (gäller BÅDE registrerade och guest-hostade spel). **Host-side-modalen nedan är därmed dormant** — approval-statusen visas i stället som grå Multiplayer Game-rad (`lockedLocalTypes`) + statusrad i utfällningen. Sync-events, `playAgainApprovals`-räkningen och non-host-halvan (`handleApprovePlayAgain`, "Approve / Replay") är oförändrade och LIVE.
...
- `src/components/MorseAmbientSound.tsx` — osynlig WebView som spelar mjuk ambient lobby-musik via Web Audio API. **Ackordprogression Am → F → C → G** (4 × 7 s = 28 s loop). Varje not = 3 sinusvågor detunade ±5 cent för chorus/pad-tjocklek. Lowpass-filter vid 800 Hz ger varm dov klang. 2,5 s crossfade mellan ackord = sömlösa övergångar. LFO 0,05 Hz ±7 % på master-gain = subtilt "andetag". Monteras i `LobbyScreen`'s `<SafeAreaView>` (position absolute left:-2, top:-2, utanför synlig yta) **gated på `hostMode`** — non-host spelare hör ingen ambient-ljud. `allowsInlineMediaPlayback + mediaPlaybackRequiresUserAction={false}` krävs för autoplay på iOS utan user-gesture. Stoppas automatiskt när komponenten avmonteras (navigation bort från Lobby). Ingen audio-fil behövs — toner genereras procedurellt.
