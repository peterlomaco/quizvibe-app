...
- `HcpProgress = { categories: Record<MainCategory, CategoryProgress> }`; `CategoryProgress = { hcp, windows: Record<AssistanceLevel, boolean[]>, lastPlayedISO }`. `hcp` lagras som FLYTTAL (delta ger decimal-delar, decay 0,25); DISPLAY avrundas ALLTID UPPÅT (§1.2.3) via `displayHcp` (ceil + clamp [1,99]).
