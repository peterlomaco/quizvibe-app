---
title: Terms of Service
description: The rules for using QuizVibe.
---

# Terms of Service

**Last updated:** 2026-05-23
**Effective date:** Upon app launch on the Apple App Store

These Terms of Service ("Terms") govern your use of the **QuizVibe** mobile application ("the app", "we", "us"). By downloading, installing, or using the app, you agree to these Terms. If you don't agree, please don't use the app.

QuizVibe is operated by **Peter Björklund**, an individual residing in Sweden. For questions about these Terms, contact us at: **infoquizvibe@gmail.com**.

---

## TL;DR (the short version)

- **You must be 13 or older** to use the app.
- **Play fair** — no cheating, harassment, profanity, or trying to break the game.
- **Purchases are final** unless the law in your country gives you a right to refund (we follow Apple's refund process).
- **Subscriptions auto-renew** until you cancel them in your Apple App Store account.
- **YouTube clips** play under YouTube's own terms — we don't own that content.
- **We can suspend or terminate your account** if you abuse the service.
- **Swedish law applies**, disputes go to Swedish courts.

The rest of this document is the full version.

---

## 1. Who we are and how to contact us

| | |
|---|---|
| **Operator** | Peter Björklund, individual residing in Sweden |
| **Contact email** | infoquizvibe@gmail.com |
| **App name** | QuizVibe |
| **Platforms** | Apple App Store (iOS) |

We are not a registered company; QuizVibe is currently operated as a personal project. If this changes, we will update these Terms and notify users via the app.

---

## 2. Acceptance of these Terms

By creating an account, joining a game as a guest, or otherwise using the app, you confirm that:

- You have read and agree to these Terms.
- You have read and agree to our [Privacy Policy](../privacy/).
- You are at least 13 years old (see Section 4).
- You have the legal capacity to enter into this agreement under the laws of your country.

If you are using the app on behalf of someone else (e.g., a parent setting up a device for a 13+ teenager), you confirm you have authority to accept these Terms on their behalf.

---

## 3. Apple's Standard EULA also applies

In addition to these Terms, your use of QuizVibe on iOS is governed by **Apple's Standard End User License Agreement (EULA)** for licensed apps, which is incorporated by reference. You can read it here: https://www.apple.com/legal/internet-services/itunes/dev/stdeula/

Where these Terms and Apple's Standard EULA conflict, **Apple's Standard EULA controls for matters relating to the iOS license itself** (e.g., installation, device limits, software restrictions). Our Terms control for matters specific to QuizVibe (gameplay, content, accounts, our purchases).

---

## 4. Eligibility and age restrictions

**You must be 13 years of age or older to use QuizVibe.**

The app enforces this by limiting the birth-year picker in registration so that no user younger than 13 can complete signup or guest entry. If we discover that a user under 13 has registered (e.g., by misrepresenting their age), we will terminate the account and delete associated data — see our [Privacy Policy, Section 7](../privacy/#7-age-restrictions-and-childrens-privacy).

If you are between 13 and 18 (or the age of majority in your country), you confirm that a parent or legal guardian is aware of and approves your use of the app.

---

## 5. Your account

### 5.1 Registered accounts

When you register, you provide a player name, email, password, birth year, and a few profile settings. You are responsible for:

- Providing accurate information (especially your real birth year).
- Keeping your password confidential.
- All activity that happens under your account.

Notify us immediately at infoquizvibe@gmail.com if you suspect unauthorized access.

### 5.2 Guest accounts

You can also join games without registering, as a "guest". Guests are identified by an anonymous session ID and do not provide email/password. Guest accounts are temporary and may be deleted when you leave a lobby or after a period of inactivity.

### 5.3 One person, one account

Each registered account is for one individual. Do not share your account, create multiple accounts to circumvent restrictions, or impersonate another person.

### 5.4 Player name rules

Player names must:

- Follow the format described in the app (letters + optional digits, max 10 letters + 7 digits).
- **Not contain** profanity, slurs, hate symbols, sexually explicit terms, or impersonation of real public figures.
- **Not violate** the rights of third parties (e.g., brand names you don't own, trademarks).

We reserve the right to reject or change a player name that violates these rules without notice.

---

## 6. Acceptable use

When using QuizVibe, you agree **not to**:

- **Cheat** — use automated tools, modify the app, exploit bugs, or otherwise gain an unfair advantage.
- **Harass or abuse** other players — including via player names, in-game behavior, or any other channel within the app.
- **Reverse-engineer, decompile, or extract** the app's source code, except where explicitly allowed by applicable law.
- **Scrape, copy, or republish** quiz content, image assets, or other materials from the app.
- **Use the app for illegal purposes** or to violate the rights of others.
- **Interfere with the service** — DDoS attacks, attempts to overload the backend, probing for security vulnerabilities without authorization.
- **Resell or commercialize** access to the app, host games for money, or otherwise monetize the service in ways we haven't authorized.

Violation of these rules may result in account suspension or termination (Section 12).

---

## 7. Content in the app

### 7.1 Quiz content (questions, clips, images)

QuizVibe presents quiz questions that include:

- **YouTube video clips** — these are streamed directly from YouTube via their official IFrame Player. **The clips themselves are owned by their respective rights holders** (artists, labels, studios, sports leagues). We do not own this content; we link to it under YouTube's terms.
- **Images of public figures, landmarks, etc.** — sourced primarily from Wikipedia/Wikimedia Commons under their respective licenses (typically Creative Commons or public domain), with attribution preserved where required.
- **Question text and gameplay structure** — written and designed by us. These are our intellectual property.

Your use of YouTube content within QuizVibe is also governed by **YouTube's Terms of Service**: https://www.youtube.com/t/terms. If you don't agree to YouTube's terms, you cannot use the parts of QuizVibe that play YouTube clips.

### 7.2 No download or extraction

You may not download, save, screen-record, or extract quiz content from the app for use outside QuizVibe. This includes YouTube clips, images, and question text.

### 7.3 Reporting copyright concerns

If you believe content in the app infringes your copyright, contact us at **infoquizvibe@gmail.com** with the subject "Copyright:" and include:

- A description of the copyrighted work you claim has been infringed.
- The specific item in the app (item ID or question text if possible).
- Your contact information.
- A statement that you have a good-faith belief that the use is not authorized.
- A statement, under penalty of perjury, that the information is accurate and that you are the rights holder or authorized to act on their behalf.

We will respond within a reasonable time and remove infringing content if the claim is valid.

---

## 8. In-app purchases

### 8.1 What you can buy

QuizVibe offers the following one-time and recurring purchases via Apple StoreKit:

- **Host Game Credits** — one-time purchases (e.g., 5, 10, or 20 credits) that let you host additional games beyond your daily free allowance.
- **Customized Host Packages** — one-time purchases that unlock themed content packages (e.g., Hip Hop, Rock, Film & Actors).
- **QuizVibe membership subscriptions** — monthly, quarterly, half-yearly, or annual auto-renewing subscriptions that unlock multiplayer features (Individual Devices mode, up to 12 players, up to 20 rounds, etc.).

Prices are displayed in the app at the time of purchase, in your local currency as provided by Apple.

### 8.2 How payment is processed

**All payments are processed by Apple**, not by us. We never see your credit card, Apple ID password, or billing address. Apple sends us only a transaction confirmation indicating which product you bought.

By making a purchase, you also agree to Apple's payment terms: https://www.apple.com/legal/internet-services/itunes/

### 8.3 Subscriptions auto-renew

QuizVibe membership subscriptions **automatically renew** at the end of each billing period unless you cancel at least 24 hours before the renewal date. The renewal charge is taken from your Apple ID account.

**To cancel a subscription:**
1. Open the **Settings** app on your iPhone/iPad.
2. Tap your **Apple ID** at the top.
3. Tap **Subscriptions**.
4. Find QuizVibe and tap **Cancel Subscription**.

Cancellation takes effect at the end of the current billing period — you keep premium features until then but won't be charged again.

### 8.4 Refunds

All purchases are processed by Apple and refund requests must be made through Apple, not us. To request a refund:

- Visit https://reportaproblem.apple.com/
- Sign in with your Apple ID.
- Find the QuizVibe purchase and request a refund.

Apple has sole discretion over refund approval. We do not control the outcome.

**Statutory consumer rights:** If you are in the EU or another jurisdiction that grants statutory refund rights for digital content (e.g., the EU 14-day right of withdrawal under the Consumer Rights Directive), those rights apply where legally required. Note that EU law allows the right of withdrawal to be waived once digital content delivery begins with your explicit consent — by default, opening or using a purchased item in QuizVibe is treated as such consent.

### 8.5 Virtual items have no real-world value

Host Game Credits, package unlocks, and subscription benefits are **virtual items** with no cash value. You cannot exchange them for money, transfer them between accounts, or claim ownership of them outside the app. If your account is terminated (Section 12), unused virtual items are forfeited.

### 8.6 Daily free credits

Registered users receive a daily allowance of free Host Game Credits (currently 2, refreshing at midnight Central European Time). This allowance may change over time at our discretion. We will give reasonable notice before reducing it.

---

## 9. Intellectual property

### 9.1 Our IP

The QuizVibe app, including its visual design, brand name "QuizVibe", logo, question text, gameplay mechanics, and original code, is owned by us and protected by copyright, trademark, and other intellectual property laws.

You receive a **limited, personal, non-exclusive, non-transferable, revocable license** to use the app for non-commercial entertainment purposes, subject to these Terms.

This license does **not** grant you the right to:
- Copy, modify, or create derivative works of the app.
- Distribute, sublicense, or resell the app or any portion of it.
- Use our trademarks or branding without written permission.

### 9.2 Third-party IP

Quiz content includes references to people, songs, films, places, and events that are the intellectual property of others. We use this content under fair use, fair dealing, public-domain, or Creative Commons licenses as applicable. **Nothing in the app implies endorsement by, or affiliation with, the people, brands, or rights holders depicted.**

### 9.3 Open-source components

QuizVibe includes open-source libraries (React Native, Expo, Supabase client, etc.) that are licensed under their respective open-source licenses. A list of these libraries and their licenses is available on request via infoquizvibe@gmail.com.

The QuizVibe app's own source code is hosted on GitHub at https://github.com/peterlomaco/quizvibe-app and is visible publicly, but its visibility does **not** constitute an open-source license — see Section 9.1.

---

## 10. Disclaimers and warranties

### 10.1 The app is provided "as is"

QuizVibe is provided **"as is"** and **"as available"**, without warranties of any kind, whether express or implied, including (but not limited to) implied warranties of merchantability, fitness for a particular purpose, or non-infringement.

We do not warrant that:
- The app will be uninterrupted, error-free, or secure.
- Quiz content is accurate, complete, or up-to-date (we do our best, but mistakes happen).
- YouTube clips will always be available (clips can be taken down by rights holders at any time).
- The app will be compatible with future iOS versions or devices.

### 10.2 You use the app at your own risk

You are responsible for your own device, internet connection, and data usage. We are not responsible for:
- Mobile data charges incurred while using the app.
- Battery drain from video playback.
- Loss of game progress due to device failure, app crashes, or your own actions (e.g., uninstalling the app).

### 10.3 Statutory consumer rights are not affected

Nothing in this Section 10 limits or excludes any warranty, condition, or other right that you have under mandatory consumer protection laws in your country (e.g., the Swedish Consumer Sales Act for residents of Sweden, or equivalent statutes in your jurisdiction).

---

## 11. Limitation of liability

To the maximum extent permitted by applicable law:

- We are **not liable** for indirect, consequential, incidental, special, or punitive damages arising from your use of the app — including loss of data, loss of profits, business interruption, or emotional distress.
- Our **total liability** for any claim related to the app is limited to the amount you paid us for the app in the 12 months preceding the claim. For free users, this is zero. For paying users, this is the total of your purchases.
- The above limitations **do not apply** to damages caused by our gross negligence or intentional misconduct, or to liability that cannot be limited under mandatory law (e.g., consumer protection law in Sweden and the EU).

---

## 12. Suspension and termination

### 12.1 When we can suspend or terminate your account

We may suspend or terminate your account, with or without notice, if you:

- Violate these Terms (including Section 6 acceptable use).
- Abuse other users, attempt to defraud us, or use the app for illegal purposes.
- Have not used your account for an extended period (12+ months) — in which case we may delete the account and send a notice to the email on file before doing so.

For minor violations, we will typically warn you first. For serious violations (fraud, harassment, security attacks), we may terminate immediately.

### 12.2 What happens when an account is terminated

When your account is terminated:
- Your profile data and game history are deleted (subject to retention requirements in our Privacy Policy).
- Unused virtual items (Host Game Credits, package unlocks) are forfeited.
- Active subscriptions are not refunded for the unused portion (cancellation must be done via Apple).
- You lose access to the app's premium features.

### 12.3 You can terminate at any time

You can stop using the app and delete your account at any time, directly in-app: open **Profile → tap your name in the top-right → Delete Account**. Server-side data is removed automatically. If you need help, email **infoquizvibe@gmail.com** with the subject "Account deletion request". See our [Privacy Policy, Section 6](../privacy/#6-your-rights-under-gdpr) for what gets deleted.

---

## 13. Changes to these Terms

We may update these Terms from time to time. When we do:

- The "Last updated" date at the top of this document will change.
- For **material changes** (changes that affect your rights, prices, or core obligations), we will notify you via an in-app message and require you to acknowledge before continuing to use the app.
- For **minor changes** (typo fixes, clarifications, adding new third-party services without changing data use), we will not actively notify but the updated version will be available at the same URL.

If you don't agree with material changes, you should stop using the app and may request account deletion before the new Terms take effect. Continued use of the app after a material change constitutes acceptance.

---

## 14. Governing law and disputes

### 14.1 Governing law

These Terms are governed by the **laws of Sweden**, without regard to its conflict-of-laws principles. This choice of law does not deprive you of the protection of any mandatory consumer protection laws of the country where you reside.

### 14.2 Court of jurisdiction

Any dispute arising from these Terms or your use of the app will be brought before **Stockholms tingsrätt** (Stockholm District Court) as the court of first instance, unless mandatory consumer protection laws in your country require otherwise.

### 14.3 Try email first

Before filing a legal claim, please try to resolve disputes by emailing **infoquizvibe@gmail.com**. We aim to respond within 30 days and resolve most issues informally.

### 14.4 EU online dispute resolution

If you are a consumer in the EU, you have the right to use the European Commission's Online Dispute Resolution platform: https://ec.europa.eu/consumers/odr/

---

## 15. Miscellaneous

### 15.1 Entire agreement

These Terms, together with our [Privacy Policy](../privacy/) and Apple's Standard EULA, constitute the entire agreement between you and us regarding QuizVibe and supersede any prior agreements.

### 15.2 Severability

If any provision of these Terms is found unenforceable, the remaining provisions remain in effect.

### 15.3 No waiver

Our failure to enforce a provision of these Terms is not a waiver of our right to enforce it later.

### 15.4 Assignment

You may not assign your rights or obligations under these Terms without our written consent. We may assign these Terms (e.g., if we ever incorporate QuizVibe as a company) with notice to you.

### 15.5 No third-party beneficiaries

These Terms are between you and us. **Apple Inc. is a third-party beneficiary** of these Terms only with respect to your use of the app on iOS — Apple has the right (but not the obligation) to enforce the provisions that govern the iOS-specific aspects of the app.

### 15.6 Force majeure

We are not liable for failure to perform under these Terms due to events beyond our reasonable control, including but not limited to: internet outages, third-party service failures (Supabase, Apple, YouTube), natural disasters, pandemics, or government actions.

---

## 16. Contact

For all questions about these Terms:

**Email:** infoquizvibe@gmail.com
**Subject line:** Please prefix with "Terms:" for faster routing (e.g., "Terms: refund question")

For privacy-related inquiries, see our [Privacy Policy](../privacy/).

---

*These Terms of Service are provided in English. A Swedish translation may be added in a future version.*
